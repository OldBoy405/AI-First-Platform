...
2. **状态数口径**：状态机 = **15 个具名状态 + 注册前 `(new)`**（口语"16 态"含 (new)）；转移 = **25 条声明，wildcard 展开后 47 条**（CR-2026-022 新增两条 reject 转换：`approve-requirement:reject -> write-requirement-prd`、`approve-dev-start:reject -> write-dev-plan`）。写文档/断言/DDL 注释时必须写明用的是哪个口径。
