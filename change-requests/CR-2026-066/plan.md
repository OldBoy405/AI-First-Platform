...
3. `cmd-01` 全量套件 `verdict=pass` / `failures=0` / `files_executed=21` / `skipped_file_level=0` / **`cases_executed ≥ 594`**（588 基线 + 6 条 AC-8 用例）；`cmd-02`/`cmd-03`/`cmd-04` 各自 exit 0，且 `cmd-04` 与 `cmd-05` 的 AC-8 存在性数值（命中用例数 ≥ 6）同批一致；`cmd-03` 的 5 个既有用例名由 `cmd-05` 的源码级守卫**逐个**判定（同批绿，表注④）——`cmd-03` 的退出码不含存在性证据。
...
| FR-3 发布与评审对象对账（AC-3） | §4.3 取证链（KB/非 KB 分解）＋逐阶段判据（`subject-sha256` / composite / `release-subjects`）＋ `CONTRACT_DRIFT` 失败语义（不改 verdict） | CR-2026-066-TASK-02 | cmd-02（`CONTRACT_DRIFT` 与「不改 verdict」token 断言 ＋ `contract-scan` 的 `RETIRED_RECOVERY` 零命中） | RU2 |
| FR-4 审批后 checkpoint 节点退役（AC-1） | §4.4.1 删除表（`…0007`/`…0005`/`…0012` 对象级删除，不重编号、不新增替代节点） | CR-2026-066-TASK-01 | cmd-02（节点数 5/4/12 + 无门后 push-progress + 被删 id 零出现）、cmd-03（连带既有断言）＋ cmd-05（5 个用例名 token 守卫） | RU1 |
| FR-5 冗余 checkpoint 节点退役与连带面（AC-1、AC-2） | §4.4.1（`…0003`×2、`…0008`、`…0015` ＋ 输入 `auto_push_*`）＋ §4.4.2 连带面 1-6（`_index.yml`、replayNodes 5→4、`approvalPrompt` 前提句、四个测试文件） | CR-2026-066-TASK-01 | cmd-02（`ref=push-progress` 计数 0 + `_index.yml` ≡ JSON + 12 节点 + inputs）、cmd-03（`CR-2026-042 静态合同` / `checkpoint T05 contract` 定点）＋ cmd-05（5 个用例名 token 守卫，表注④） | RU1 |
...
④ `cmd-03` 的 `--test-name-pattern` **空跑即绿**（本条 run 实测，非推断）：① 同一 args 实跑 stdout 只有 **10 个点号**——dot 报告器**不含用例名、不含摘要行**；② `--test-name-pattern ZZZ-no-such-case-9999` ⇒ **exit 0、无任何告警**。故 `cmd-03` 的**退出码与点号数都不构成存在性证据**（某个名字消失后点号数仍可 ≥ 10）。5 个既有用例名的存在性由 `cmd-05` 的**源码级守卫（g）**承载：**3 个文件 × 5 个 token** 逐个断言**声明形态** `test('<token>` 的出现次数 ≥ 1 —— `crctl.test.mjs` 的 `CR-2026-042 静态合同`、`checkpoint-tx.test.mjs` 的 `checkpoint T05 contract`、`pipeline-structure.test.mjs` 的 `CR-2026-044 AC-13`（该文件内现有两处声明命中该 token：用例名 `CR-2026-044 AC-13/14: …` 与 `CR-2026-044 AC-13: …`；守卫只要求 ≥ 1，不锁数量）、`CR-2026-044 AC-14`、`CR-2026-050 AC-12`；任一 token 丢失 ⇒ `cmd-05` 红（逐个独立判定，不用跨文件合计数）。**token 前缀必须逐字保留**：TASK-01 改写只改断言体；后缀描述若因新事实失真（如 `code Pipeline 16 节点` ↔ 12 节点、`…强制 checkpoint（7 节点）…` ↔ 5 节点、`5 节点不变` ↔ 4 节点、`16 节点不变` ↔ 12 节点）则同步更新为新事实，但**不得改动 token 前缀**（改前缀即守卫红）。`cmd-03` 与 `cmd-05` 必须**同批判读**（TASK-01 完成标志含此项）。
⑤ **`cmd-04` 的假绿口子由两道判据闭合**：① **用例名约定**（TASK-03 的 AC-8 六项用例名必须以 `CR-2026-066 AC-8` 起始）+ `cmd-05` 的**源码级存在性守卫**（`CR-2026-066 AC-8` token ≥ 6、`test(` 计数 ≥ 30）；② `cmd-04` 非零退出即红。**本条 run 实测：缺用例时 `--test-name-pattern` 空跑 exit 0（无摘要）** ⇒ 单靠 `cmd-04` 的退出码**不构成存在性证据**，必须与 `cmd-05` 的守卫同批判读；`test-report.md` 分析段与 TASK-03 完成标志须逐字记录**两处数值，且不得互相替代**：① `archive-tx.test.mjs` 内 `CR-2026-066 AC-8` 的 **token 计数**（≥ 6）与 `test(` 调用计数（≥ 30）——`cmd-05` 的源码级守卫实测值（存续性证据）；② `cmd-04` 的 **dot 点数**（= 命中且通过的用例数，期望 ≥ 6；dot 报告器无摘要行）。② 只是「有若干用例跑了」的计数，**不能**替代 ① 的名字级证据。
...
覆盖三类判据：**(a)** `plan.md` 的交付说明必填块（AC-6 六字段、FR-11 的 `AIFIRST_ARCHITECTURE_RUNNER` 与「未重生成」分支、D-6 的第五份副本排除理由、部署窗口成对生效、断言 B 结论的两个 token）——**正向 token 的扫描面限定在 §10 区块内**（`## 10. ` 起至下一个 `## ` 标题；区块缺失或区块行数 < 10 即硬失败），不再用全文：全文面会被 §6.2.1~§6.2.3 的判据说明行自证（本轮实测：16 个正向 token 中 **12 个**在 §10 之外有命中，仅 `观察项 ①`~`④` 只在 §10 出现）；**(b)** 不得出现「重生成已完成」或平台侧部署声称（负向，**仍按剔除 `-e` 证据行后的全文**）；**(c)** KB diff 面 ⊆ `change-requests/CR-2026-066/**` ∪ `change-requests/_backlog.yml`。
...
| cmd-03 | 可达 | **exit 0**（10 个点号，纯静态用例 < 1 s；**stdout 无用例名与摘要**）；同批实测空跑：`--test-name-pattern ZZZ-no-such-case-9999` ⇒ exit 0、无告警 | 变更前绿；变更后须仍绿（断言体改新事实）；**存在性不由本条退出码承担**——由 `cmd-05` 的守卫（g）（3 文件 × 5 token，表注④）同批兜底 |
...
| cmd-07 | 可达 | **exit 0**（登记块已在 §10 落盘 ⇒ AC-6 按「登记即达成」定义在交付时即可判定；`section10` 区块 35 行、16 个正向 token 全在其中）；**失败向量已实测**：首版负向判据命中 2 项（禁止声称），修正表述后归零；**正向面活性已实测**：全文扫描时 16 个正向 token 有 **12 个**可被 §10 之外的判据说明行自证 ⇒ 本轮把正向扫描面收窄为 §10 区块（§6.2.3） | 变更后须继续 exit 0（§10 区块存在 ∧ 16 个正向 token 齐备 ∧ 负向判据零命中） |
...
|---|---|---|---|
| AC-1 节点数 5/4/12；无门后 `push-progress`；`ref=push-progress` 计数 = 0；`_index.yml` ≡ JSON；被删 id 零出现 | §4.4.1 ＋ §6.2 AC-1 | CR-2026-066-TASK-01 | cmd-02；cmd-03；cmd-05（5 个既有用例名 token 守卫，表注④） |
| AC-2 既有断言按新事实同步且全绿；新增断言覆盖两条判据与「review SKILL 含发布步骤 + clean 前置」 | §6.4 ＋ §4.4.2-4 ＋ NFR-5 | CR-2026-066-TASK-01 | cmd-02；cmd-03；**cmd-05（`crctl.test.mjs` / `checkpoint-tx.test.mjs` 的 3 个 token 守卫——这两个文件不整文件进证据集，其「断言已按新事实同步」的定点机械证据即本条 ＋ cmd-03；表注③/④）**；cmd-01（全套件不回归） |
...
| AC-10 ①scope_out 可检查（无平台执行层/新节点维度账本字段、无事务层与状态机改动、无 `recoverCommand` 复活）；②不做「审批后可选 checkpoint」；③串行（在途唯一）；④与 CR-P1/P2 面零 diff | §9 `scope_out`/`zero_diff` ＋ §7 | CR-2026-066-TASK-04 | cmd-05（tools diff/zero_diff + hunk 级禁改 token）；cmd-06（multica diff）；cmd-07（KB diff）；cmd-02（`contract-scan` 的 `RETIRED_RECOVERY` 零命中，与 §6.1 FR-3 行同归属） |
...
### E. `review-dev-plan` 评审（cycle 1 / attempt 1 BLOCK）的回修登记

评审者判 **BLOCK**（8 维中唯 `acceptance-verifiability` = block，其余 7 维 pass），`repair-target = write-dev-plan`，`reviewLoop.replayNodes = write-dev-plan → write-dev-tasks → review-dev-plan`（≤3 轮）。本轮回修在同一批内改 `plan.md` ＋ 四张 TASK 卡，逐条如下（“本轮复测”= 在本计划指定的 tools worktree 上只读实测）：

| # | 来源 | 判定事实（本轮复测，非转述） | 处置 | 落点 |
|---|---|---|---|---|
| **B-1** | blocker | `cmd-03` 的 5 个既有用例名**无存在性守卫**：① 同一 args 实跑 stdout 只有 **10 个点号**（无名字、无摘要）⇒ TASK-01 §4.2 原「stdout 命中数 ≥ 5」不能证明其声称的结论（某个名字消失后计数仍可 ≥ 10）；② `--test-name-pattern ZZZ-no-such-case-9999` 空跑 **exit 0 无告警**；③ 原表注④ 声称的「改名即红」在本计划的命令形态下**不存在实现**，与表注⑤ 自测的空跑即绿互相矛盾；④ 原 `cmd-05` 守卫只盖 `archive-tx.test.mjs` 的 AC-8 token 与 `test(` 计数 | 在 `cmd-05` 增加**源码级存在性守卫（g）**：3 个文件 × 5 个 token，逐个断言**声明形态** `test('<token>` ≥ 1（现有 5 个 token 实测均存活 ⇒ 该守卫把变更前基线仍固定在 42 failures，不引入新基线）；表注④ 按机械判红通道重写（不再声称不存在的机制）；TASK-01/TASK-03 的验收步骤改为「exit 0 ∧ `cmd-05` 守卫同批绿」；完成记录改记守卫的 token 清单与计数（dot 约定不变，不新增报告器） | `plan.md` §6.1 表注④、§6.2 `cmd-05` args、§6.2.1(g)、§6.3、§5.1-3、§7 AC-1/AC-2、§9；`tasks/TASK-01.md` §4.2/§5；`tasks/TASK-03.md` §4.1/§5 |
| **S-1** | suggestion | `RETIRED_RECOVERY` 零命中的唯一载体是 `contract-scan.test.mjs` 的整树扫描（`RETIRED_RECOVERY = ['recoverCommand','recover_command']` ＋ `enumerateWorktreeAt`，L418-425 区域），即 `cmd-02`；`cmd-04` 只跑 `archive-tx` 的 `CR-2026-066` 用例 | 两处归属 `cmd-04` → `cmd-02`（与 §7 AC-3 行、SDD §6.2 AC-10 行一致；7 条命令同批执行，验收可达性不变） | `plan.md` §6.1 FR-3 行、§7 AC-10 行 |
| **S-2** | suggestion | 16 个正向 token 中 **12 个**在 §10 之外有命中（`AC-6 延期验证点` / `载体` / `时点` / `责任 agent` / `关闭触发` / `AIFIRST_ARCHITECTURE_RUNNER` / `未重生成` / `D-6` / `cr-prompts-revised/agent-skill-matrix.yml` / `部署窗口` / `受限 crctl 权限` / `workspace inspect`），其中 §6.2.2/§6.2.3 的判据说明行即可自证（不属被测面）；只有 `观察项 ①`~`④` 仅在 §10 出现 | `cmd-07` 的**正向 token 扫描面收窄为 §10 区块**（`## 10. ` 起至下一个 `## ` 标题；区块缺失或区块行数 < 10 ⇒ 硬失败）；**负向面仍按剔除 `-e` 证据行后的全文**（口径不改） | `plan.md` §6.2 `cmd-07` args、§6.2.3、§6.3 |
| **S-3** | suggestion | `tasks/TASK-04.md` §2 表的文件面实为 **13 个**（10 行展开：5 处文档 ＋ tools 三份 Agent Prompt ＋ multica 四份副本 ＋ `contract-scan.test.mjs`；按仓 = tools 9 ＋ multica 4），§5 误写「11 个文件」 | 数字改 13 并给出逐文件分解与仓分布 | `tasks/TASK-04.md` §5 |
| **S-4** | suggestion | `tasks/TASK-01.md` 计数三处不一致：§2/§3.2 标题写「9 处」、§3.2 逐行表实为 **12 行**（11 改写 ＋ 1 保留）、§5 又写「SDD §6.4 的 14 处」 | 口径统一为**按 SDD §6.4 表格行**计数：`pipeline-structure.test.mjs` 既有断言 **12 行**（＋1 行新增），四测试文件既有断言改动合计 **15 行**（12 ＋ 1 ＋ 1 ＋ 1）；§2/§3.2/§5 同口径改写，防按错数自查漏改。**与评审意见的 14 的差异及依据**：SDD §6.4 对 `pipeline-structure` 逐行实为 **13 行**（12 行既有断言 ＋ 1 行 `**新增**`；本轮逐行复算），评审意见的 14 系把该文件按 11 行既有断言计；差异仅在 `pipeline-structure` 的行数，且两方口径都可从 SDD §6.4 表行复算——本轮取「表行数」口径并逐行留档 | `tasks/TASK-01.md` §2、§3.2、§5 |
| **S-5** | suggestion | SDD §4.7 把 dev-agent 的两句旧前提句登记在 tools 的 `agents/dev-agent.md`；实测该文件（70 行）**零** `checkpoint` 字样，两句只在 multica `cr-prompts-revised/dev-agent.md`（L23/L41）；SDD §6.3 第 31/34 项的实测结论与 §4.7 表行相反 | **在本表登记该 SDD 落点错位**（不改 `sdd.md`，审批冻结）：交付面以 §0.6（实测事实）、TASK-04 §3.3 与 `cmd-06`（multica 落点）为准 | 本表（§8.E）；`tasks/TASK-04.md` §3.3 已按实测落 multica |
| **S-6** | suggestion | `tasks/TASK-01.md` §3.3 ② 的「被删的 7 个 **id 后缀**…零出现」按字面**恒假**：`…0003` / `…0005` 等后缀在其它 pipeline 的存活节点上仍存在（如 architecture `00000000-0000-0000-0016-000000000003` = `human_approval`、code `…0015-000000000003` = `push-progress`） | 断言改为**按完整节点 id**（7 个 id 逐字列出）零出现，并写明「只按后缀断言会得到恒假结果」 | `tasks/TASK-01.md` §3.3 ② |

**同批的表述澄清（非评审意见，防下一轮返工）**：「保留用例名」的精确口径 = **保留用例名的 token 前缀**（`CR-2026-042 静态合同` / `checkpoint T05 contract` / `CR-2026-044 AC-13` / `CR-2026-044 AC-14` / `CR-2026-050 AC-12`）——`cmd-03` 的 `--test-name-pattern` 与 `cmd-05` 守卫（g）都只依赖该前缀；后缀描述若因新事实失真（既有名 `…code Pipeline 16 节点…` ↔ 新事实 12 节点、`…强制 checkpoint（7 节点）…` ↔ 5 节点、`…5 节点不变` ↔ 4 节点、`…16 节点不变` ↔ 12 节点），改写时同步更新为新事实，**不得改动前缀**（改前缀即守卫红）。
...
- 命名约定：TASK-03 新增的 `test(...)` 名必须以 `CR-2026-066` 起始（`cmd-04` 的定点模式依赖，存在性由 `cmd-05` 守卫（f）承载）；TASK-01 改写既有断言时**保留用例名的 token 前缀**（`cmd-03` 的定点模式依赖，存在性由 `cmd-05` 守卫（g）**逐个**机械判定；后缀描述可随新事实更新，前缀不得改）。
...
- 回修 0.2（2026-09-14，`review-dev-plan` cycle 1 / attempt 1 BLOCK → 按 `repair-target=write-dev-plan` 回修）：**B-1** 把 `cmd-03` 的 5 个既有用例名的存在性从「退出码/点号数」改为 **`cmd-05` 新增的源码级守卫（g）**（3 个文件 × 5 个 token × 声明形态 `test('<token>` ≥ 1，逐个判定），表注④ 按机械判红通道重写（原文声称的「改名即红」不成立：实测 `--test-name-pattern` 空跑 exit 0、dot 报告器只出点号），TASK-01/TASK-03 的验收步骤与完成记录同步改为「`cmd-05` 守卫同批绿 ＋ 记 token 清单与计数」；同批处置 6 条 Suggestions（§8.E）：`RETIRED_RECOVERY` 归属 `cmd-04`→`cmd-02`、`cmd-07` 正向扫描面收窄至 §10 区块、TASK-04 计数 11→13、TASK-01 计数口径统一（SDD §6.4 既有断言 15 行）、SDD §4.7 落点错位登记（不改 SDD）、TASK-01 §3.3 ② 改按全节点 id 断言；并澄清「保留用例名」= 保留 token 前缀（后缀随新事实更新）。证据（均在 tools worktree 只读实跑）：新 `cmd-05` 脚本在变更前清工作区上 = **exit 1 / 42 failures**（与回修前基线逐项一致，新增 5 项守卫全绿并逐行打印计数）；守卫谓词负控（改名即红）在临时副本上实测；新 `cmd-07` 脚本 = exit 0（`section10` 35 行、16 个正向 token 齐备、负向零命中）。
