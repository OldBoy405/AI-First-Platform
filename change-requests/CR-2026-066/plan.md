...
| **S-4** | suggestion | `tasks/TASK-01.md` 计数三处不一致：§2/§3.2 标题写「9 处」、§3.2 逐行表实为 **12 行**（11 改写 ＋ 1 保留）、§5 又写「SDD §6.4 的 14 处」 | 口径统一为**按 SDD §6.4 表格行**计数：`pipeline-structure.test.mjs` 既有断言 **12 行**（＋1 行新增），四测试文件既有断言改动合计 **15 行**（12 ＋ 1 ＋ 1 ＋ 1）；§2/§3.2/§5 同口径改写，防按错数自查漏改 | `tasks/TASK-01.md` §2、§3.2、§5 |
