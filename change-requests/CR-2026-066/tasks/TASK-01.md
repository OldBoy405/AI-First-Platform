...
| `skills/shared/crctl/scripts/test/pipeline-structure.test.mjs` | 改 11 处 ＋ 保留 1 处 ＋ 新增 AC-1 断言（合并覆盖 SDD §6.4 对本文列的 **12 行**既有断言） | 见 §3.2 的 12 行台账（11 改写 ＋ 1 保留）；**保留既有用例名的 token 前缀**（`cmd-03` 的定点模式依赖；存在性由 `cmd-05` 守卫（g）**逐个**机械判定，见表注④） |
| `skills/shared/crctl/scripts/test/crctl.test.mjs` | 改 1 处用例 | `CR-2026-042 静态合同：…`：`inputs` → `['cr_id','target_version']`；节点数改由事实源推导（12）；**保留 `…0017` 直接前驱 `…0009`**；**保留用例名 token 前缀** `CR-2026-042 静态合同`（后缀 `code Pipeline 16 节点` 按新事实改 `12 节点`；存在性由 `cmd-05` 守卫（g）判定） |
| `skills/shared/crctl/scripts/test/checkpoint-tx.test.mjs` | 改 1 条 test 的 L491 | 见 §3.4；**保留用例名 token 前缀** `checkpoint T05 contract`（存在性由 `cmd-05` 守卫（g）判定） |
...
### 3.2 `pipeline-structure.test.mjs` 的 12 行处置台账（11 改写 ＋ 1 保留；逐条对应 SDD §6.4 的 12 行既有断言）
...
| 6 | `CR-2026-044 AC-13/14: requirement-authoring 审批后强制 checkpoint（7 节点），草稿 checkpoint 仍可选`（L168-183） | **改写**：5 节点；**「approve-requirement 之后不得有 push-progress」**；`_index.yml#nodes = 5`；删草稿 checkpoint 分支断言。**用例名**：token 前缀 `CR-2026-044 AC-13` 保留，后缀 `（7 节点）` 按新事实改 `（5 节点）`，`草稿 checkpoint 仍可选` 段随断言删除一并去掉 |
| 7 | `CR-2026-044 AC-14: architecture-design 删除 auto_push_after_sdd，审批后 checkpoint abort，5 节点不变`（L186-203） | **改写**：`push.length === 0`、4 节点；保留「prompt 无 `crctl checkpoint` 字面量」与 `<installation-workspace>` 负向断言（对象消失后按事实源推导重写）。**用例名**：token 前缀 `CR-2026-044 AC-14` 保留，后缀 `5 节点不变` → `4 节点` |
| 8 | `CR-2026-044 AC-13: code-implementation 审批后 checkpoint abort、TASK checkpoint 仍可选、16 节点不变`（L205-213） | **改写**：`ref=push-progress` 计数 = 0 + 12 节点 + `inputs` 不含 `auto_push_after_task`。**用例名**：token 前缀 `CR-2026-044 AC-13` 保留，后缀 `16 节点不变` → `12 节点` |
...
② 每份 JSON 中：不存在任何位于 human_approval 之后的 ref=push-progress 节点（按节点序求值）；
   ...
   且被删的 7 个**完整节点 id**（`00000000-0000-0000-0011-000000000003`、`00000000-0000-0000-0011-000000000007`、`00000000-0000-0000-0016-000000000005`、`00000000-0000-0000-0015-000000000003`、`00000000-0000-0000-0015-000000000008`、`00000000-0000-0000-0015-000000000012`、`00000000-0000-0000-0015-000000000015`）在任何 pipeline JSON 中零出现。
   **必须按完整 id 断言**（含 pipeline 段）：只按末段后缀（`…0003` 等）断言会得到**恒假**结果——这些后缀在其它 pipeline 的存活节点上仍存在（如 architecture `00000000-0000-0000-0016-000000000003` = `human_approval`、code `…0015-000000000003` = `push-progress`），该断言永远红。
```
...
- 用例名（**保留 token 前缀 `checkpoint T05 contract`**；本卡不改名，后缀描述若失真可同步更新）：`checkpoint T05 contract：Pipeline 只编排 Skill，active alignment reader 不读旧 checkpoints[]`；存在性由 `cmd-05` 守卫（g）（声明形态 `test('<token>` ≥ 1）机械判定，不依赖 `cmd-03` 的退出码。
...
2. **`cmd-03`**（plan §6.2）：定点跑 `CR-2026-042 静态合同` / `checkpoint T05 contract` / `CR-2026-044 AC-13` / `CR-2026-044 AC-14` / `CR-2026-050 AC-12` → **exit 0**，**且 `cmd-05` 的 5 个用例名 token 守卫（`cmd-03 guard …`行，3 个文件 × 5 个 token）同批全绿**（证明 5 个既有用例名的 token 前缀仍在、断言体已按新事实改写）。**禁止**以 `cmd-03` 的 stdout 命中数作为存在性证据：dot 报告器只输出点号（无名字、无摘要），且 `--test-name-pattern` 空跑 exit 0（plan §6.1 表注④）。
...
- 8 个文件就位并随 CR 提交（`[cr]` 前缀消息）；`cmd-02` / `cmd-03` exit 0；**`cmd-05` 的 5 项用例名 token 守卫全绿**（`cmd-03 guard …` 逐行计数留档）；`cmd-01` 全量套件绿（无例外）。
- SDD §6.4 的**既有断言改动行逐行核对表**（按表行计数：`pipeline-structure` **12** 行＋ `contract-scan` 1 ＋ `crctl` 1 ＋ `checkpoint-tx` 1 ＝ **15 行**；另有新增面：本文列的 AC-1/AC-4 断言、`contract-scan` 的 FR-7 静态断言、`archive-tx` 的 AC-8 六项用例）写入本任务完成记录（每行 = 断言 → 处置 → 新落点），不得遗漏任一行为「无需改」（若判定无需改，写明理由）。
