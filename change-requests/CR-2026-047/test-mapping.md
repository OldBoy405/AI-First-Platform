...
| AC-18 周报同周幂等 | `TestEnsureOrgAdminWorkspaceIdempotent` 真实 PG 覆盖 schedule dispatch→project/chat task→无效 envelope fail-closed→direct result/chat/inbox；第二个同周 retry task 复用 chat，事务 advisory lock 保证 inbox 仍为 1；文件原子落盘见残余风险 R-2 | `server/internal/service/org_admin_test.go`、`maturity_completion.go` | ✅（server 全链路）/ ⏳（真实 daemon 文件） |
| AC-19 报告五节 | `TestMaturityReportBuiltinSkillContract` 与 completion validator 双重断言五节标题和指标 key，缺节报告拒绝完成 | `server/internal/service/builtin_skills/multica-maturity-weekly-report/SKILL.md`、`org_admin_test.go`、`maturity_completion.go` | ✅ |
