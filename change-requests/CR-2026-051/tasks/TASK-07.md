...
2. **未启用形态（AC-12）**：不设 `MULTICA_LARK_SECRET_KEY` 构造 router（或直接按 wiring 形态构造 `ApprovalReminderConfig{Pool: nil, AppURL: "https://multica.test"}` 后 `Register(bus)`）→ 发布一条真事件后：进程存活、恰一条 `result=skipped reason=feishu-disabled` 事件级日志、零真实飞书请求、**零收件人查询**。
   **零查询的取证形态（架构评审 cycle 2 建议 2）**：`Pool` 是具体类型 `*pgxpool.Pool`，**不存在接口替身，也不存在“pool 替身调用计数”**，禁止写成“pool 替身零调用”；本 TASK 统一取 `Pool: nil`（查询在结构上不可能发生），并附一条正向断言：日志中**无任何 `result=failed` 行、无任何 `phase=panic-recovered` 行**（若真走到查询，nil 受体会 panic → TASK-05 要点 5 的 recover 必留下 `phase=panic-recovered`）。**不得写成“无 Error 级日志”**：本形态下 `Pool`/`Client`/`Credentials` 均为 nil，TASK-05 要点 2 规定构造期必然记一条 `phase=construct` 的 dependency-missing Error，那是**预期输出**（可反向兑现为：恰一条 `phase=construct` 且 `missing` 含三项）。取证形态与口径与 TASK-05 验收 1 一致（dev-plan 评审 attempt 1 BL-1 同家族修正）。**`phase` 闭集为三值（attempt 2 BL-1 同步）**：TASK-05 要点 9 末段已把 `Error` 级输出收编为 `construct` / `register` / `panic-recovered` 三个互斥 `phase`；本 TASK 的 `Register` 一律传**真 bus**，故 `phase=register` 行数恒为 **0**，上面那条“恰一条 `phase=construct`”的数值不受扩集影响（计数口径同样是**按 `phase` 值分别计数**，不对 Error 级总行数计数）。
