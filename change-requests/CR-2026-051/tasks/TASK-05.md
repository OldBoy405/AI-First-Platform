...
3. **订阅**：`func (r *ApprovalReminder) Register(bus *events.Bus)` → `bus.Subscribe(protocol.EventCRApprovalGateEntered, r.handleEvent)`；`bus == nil` 时记 Error 并直接返回（不 panic）。无条件可调用是 FR-8.3 的前提。
...
9. **日志入口（SDD §4.6 的四类结果；`skipped` 按作用域拆成两个函数，字段口径不变）**（字段口径按 SDD §4.6，字段名与 PRD §4.4 表一致）：
   ...
   **两条非 result 类 `Error` 日志（本 TASK 唯一的 `Error` 级输出；均不带 `result` 字段，靠 `phase` 与四类结果日志、以及彼此相互区分）**：`phase=construct`（要点 2 的 dependency-missing，附 `missing`）与 `phase=panic-recovered`（要点 5 的兜底，附 `cr_id`/`stage`/`workspace_id`/`event_id`）。`phase` 是**诊断字段而非结果分类**：它不进 PRD FR-8.2 的 9 项 `reason` 闭集、不新增第 10 个 reason，也不改 SDD §4.6 的四类结果口径。这两个可区分值是验收 1 与验收 4 断言的基础（dev-plan 评审 attempt 1 BL-1 回修：原“无 Error 日志”与本要点 2 的构造期 Error 互相矛盾，不可满足）。
...
1. **依赖缺失不 panic，且逐依赖隔离取证（AC-12 部分；dev-plan 评审 BL-3 回修）**：`Pool`/`Client`/`Credentials` 任一为 nil 时 `NewApprovalReminder` 返回非 nil。行为断言必须拆成**四个子用例，每个只置一项为 nil、其余依赖均健康**：(a) 只 `Credentials = nil`；(b) 只 `Client = nil`；(c) `Client` 非 nil 但 `IsConfigured() == false`；(d) 只 `Pool = nil`。每个子用例：`Register(bus)` + `bus.Publish(真事件)` 后进程存活、恰一条 `result=skipped reason=feishu-disabled`、**零 DB 访问**。
   ...
   **构造期 Error 是预期输出，不是断言噪音（dev-plan 评审 attempt 1 BL-1 回修）**：要点 2 规定 `Pool`/`Client`/`Credentials` 任一为 nil 时构造函数**必然**记一条 dependency-missing Error，因此本验收**不得**写成"无 Error 日志"（那与要点 2 直接冲突、按计划自身即不可满足）。断言口径改为按 `phase` 区分（要点 9 末段）：
   ...
   - **反向（四个子用例共同）**：日志中**不存在任何 `result=failed` 行**，也**不存在任何 `phase=panic-recovered` 行**。
...
4. **panic 自恢复（AC-13）**：客户端替身的 `IsConfigured()` 直接 `panic("boom")`，断言进程存活、恰一条 `phase=panic-recovered` 的 Error 级日志（口径见要点 9 末段；本用例三依赖均健康，故 `phase=construct` 行数为 0）、信号量已释放（后续事件仍能被处理）。
