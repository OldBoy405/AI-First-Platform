...
- 产出：
  ...
  - `agent_task_queue.context.chat_config` 快照格式（`{"model": string, "thinking_level": string}`）与共享 merge 实现——daemon claim 的消费契约，兼供 TASK-13 `CreateChatTask` 接缝消费（BLOCK-005），无新增导出符号。
