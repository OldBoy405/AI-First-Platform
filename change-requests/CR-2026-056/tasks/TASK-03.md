...
2. Private Ask（`chat_session`）：`PatchChatSessionConfig`（写 override）、`BackfillChatSessionBaseIfNull`（仅 `base_*` NULL 时写）。
3. 改造 `GetProjectChatSessionForCreator`：WHERE 增加权威 `workspace_id`（Hard Invariant 1，SDD §2.2）；同步改所有调用方（含 `autopilot.go`）Params，缺 workspace 不得编译通过。
4. 改造 `GetProjectChatIssue`（兼容钉，BLOCK-017）：`ORDER BY created_at ASC, id ASC LIMIT 1`，保持 `:one`；仅转投路径继续调用（SDD §4.13）。
5. 附件（SDD §2.5）：
...
6. `make sqlc` 生成；生成物禁手改（ARCHITECTURE.md Generated sources）。
...
- 产出（sqlc 生成符号，参数列集如上；Go 签名以生成物为准）：`GetActiveProjectChatSession` / `GetProjectChatSessionByID` / `LockProjectChatSessionByID` / `InsertProjectChatSession` / `PatchProjectChatSessionConfig` / `BindProjectChatSessionIssue` / `CloseActiveProjectChatSession` / `GetLegacyUnboundProjectChatIssue` / `CountProjectChatSessions` / `PatchChatSessionConfig` / `BackfillChatSessionBaseIfNull` / `LockUnboundDraftAttachments` / `BindUnboundDraftAttachments` / `DeleteUnboundDraftAttachment`，以及改造后的 `GetProjectChatSessionForCreator`（带 `workspace_id`）与 `GetProjectChatIssue`（`ORDER BY created_at ASC, id ASC LIMIT 1`）——供 TASK-06/07/08/09 消费。
