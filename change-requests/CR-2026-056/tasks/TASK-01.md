...
4. 验证 `make sqlc` 可运行；`go test ./server/internal/handler/ ./server/pkg/agent/ ./server/internal/service/ -count=1` 三组入口在基线上全绿；前端测试命令（`packages/core/api/schemas.test.ts`、locales parity、team-agent / private-ask 组件测）入口可用。
...
2. 迁移目录最大编号 471；`make sqlc` 运行成功；三组 `go test` 基线绿（输出留档）。
...
- 消费：无（首个 TASK）。
- 产出（事实，非代码）：两资源权威路径、multica 基线 SHA `8746add...`、最大迁移号 471、验证命令清单——供 TASK-02（迁移编号起点）、TASK-03（make sqlc）、TASK-11（测试命令）消费。
