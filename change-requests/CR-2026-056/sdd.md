...
updated: 2026-08-30T18:00:00+08:00
---
...
| 隐藏执行容器 Issue | `issue.origin_type='project_chat'` | GET 打开面板 vs 首次发送 | 同一实体；GET 不得创建，发送/显式创建才绑定 |
...
| `server/internal/service/project_chat.go` | GET 不再创建 Issue；抽出 `BindProjectChatContainer`（`EnsureProjectChatIssue` 后继，必须在 session 行锁下调用） | 服务层事务与幂等；handler 不复制第二套创建 |
...
显式创建 / 首次发送（与消息写者同一权限：presenter 规则）
  POST .../chat/container | POST .../chat/messages
    → 同一 BindProjectChatContainer（session FOR UPDATE）
    → 无 issue_id 则 ensureContainerIssue 并回写
    → 发送：解析有效配置 → merge chat_config → 同事务绑定附件 → 201
...
| 不变量 | 设计保证 |
|---|---|
| Workspace isolation | 所有查询带 `workspace_id`；`session_id` 必须属于当前 project |
...
### 1.6 Merge-forward 与容器唯一性

`POST /api/projects/{id}/chat/merge-forward` 复用 `sendProjectChatCore`，今天也会 `EnsureProjectChatIssue`。GET 不再创建容器后，从 Discussion 转发可能是第一次创建容器。

PRD 未要求 merge-forward 带 `session_id`（Discussion UI 不一定持有）。设计：

- merge-forward **不**新增 `session_id` 请求字段；
- 服务端先 Ensure 当前项目 active `project_chat_session`（无则按 FR-5 插入并写 `base_*`），再走同一 `BindProjectChatContainer`；
- Discussion GET/发送路径零改动（NFR-7）。

这样并发「打开面板 PATCH」与「Discussion 转发」仍最多一个 active session、一个容器 Issue。
...
| `issue_id` | UUID NULL | 空=尚未绑容器 | 首次发送或 POST container 回写 |
...
既有 `GetProjectChatSessionForCreator`（`server/pkg/db/queries/chat.sql`）**继续只服务 Private Ask `chat_session`**。
...
Private Ask：为 `chat_session` 增加 `PatchChatSessionConfig`、`BackfillChatSessionBaseIfNull`（仅 NULL 时写 base_*）。
...
| 476 | 可选：`CREATE INDEX CONCURRENTLY ... ON project_chat_session (workspace_id, project_id);`（含 closed 历史） |
| 477 | `ALTER TABLE chat_session ADD COLUMN base_model TEXT, ADD COLUMN base_thinking_level TEXT, ADD COLUMN model_override TEXT, ADD COLUMN thinking_level_override TEXT;`（单语句四列） |
...
PATCH：creator-only；`project_id IS NULL` 的普通 1:1 `chat_session` **拒绝**（本 CR 只对 Private Ask 生效）；不得改 `project_chat_session`。非创建者 `403 forbidden_chat_config`。请求体与 Team Agent PATCH 相同（无 `session_id` 字段，以路径为准）。历史缺 `base_*` 允许 `agent_default`，首次 PATCH/发送回填。
...
### 4.3 catalog + readiness（PATCH 与发送共用）
...
### 4.4 BindProjectChatContainer（幂等）

`EnsureProjectChatIssue` 的后继，**仅** container 与 messages（及 merge-forward 内部）调用；GET 禁止。
...
issue = ensureContainerIssue(...)     # 复用现 advisory lock + 部分唯一 origin_type
BindProjectChatSessionIssue CAS
return issue
```

同 `session_id` 并发 container+messages：行锁串行化，最多一个 Issue。

### 4.5 发送（Team Agent）

```text
validate session_id（同 PATCH）
BindProjectChatContainer
guard presenter + queue capacity（现 sendProjectChatCore 前半）
ResolveChatConfig + catalog/readiness（失败则无 comment）
tx:
  CreateComment
...
commit
# 失败：现补偿删除 comment；附件保持未绑定
broadcast after commit
```

**相对现状的关键差**：今日 `linkAttachmentsByIDs` 在 enqueue **成功之后**、事务外调用，且 SQL 要求 `issue_id` 已在附件上。本 CR 改为发送事务内绑定未绑定草稿；composer 上传 **省略** `issue_id`（`TeamAgentComposer.handleComposerUpload` 今日传 `{ issueId }`）。

### 4.6 Private Ask 发送

既有 chat 入队路径增加：缺 `base_*` 则一次回填；Resolve + 校验；merge `chat_config`。不经 `project_chat_session`。

### 4.7 换绑 Team Agent（FR-7）

`PATCH /api/projects/{id}` 写 `settings.team_agent_id` 的现有分支（`handler/project.go`）在提交成功后：

```text
CloseActiveProjectChatSession
# 不在此处创建新 session；下一次 GET 按新 agent_id 插入新行并复制新默认
# 旧 issue_id 仍挂在 closed session 上，只读；不得把旧 timeline 灌入新 session
```
...
新 service 函数（测试文件 `chat_draft_attachment_cleanup_test.go`）：
...
| FR-3 | Private Ask 仍按 `(project_id, creator_id)`；override 不写 Team Agent 表 |
| FR-4 | §4.3 共用校验；Waitable+cache / Waitable+no-cache / Blocked 三分支 |
| FR-5 | Insert 时写 `base_*`；Private Ask 新建同样 |
| FR-6 | §4.2 优先级与 PATCH 三态；source 与有效值一致 |
| FR-7 | §4.7 close 旧 session；GET 建新 |
| FR-8 | GET 只 Ensure session；响应含可空 `issue_id` 与 source |
| FR-9 | PATCH 不要求 `issue_id` |
| FR-10 | §4.4 共用 Bind；POST container 新路由 |
| FR-11 | `chat_session` 四列 + GET 不回写 + 首次 PATCH/发送回填 |
| FR-12 | `PATCH /api/chat/sessions/{id}/config` creator-only |
| FR-13 | enqueue merge `chat_config` |
| FR-14 | §4.8 claim 读快照；旧任务不回填 |
| FR-15 | 上传省略 issue_id；§4.9 上传者门 |
| FR-16 | 发送同事务绑定；失败保留草稿；§4.10 sweeper |
| FR-17 | §3.3 + 前端按 code 回滚控件/保留草稿 |
| FR-18 | 复用 ChatInputCore + Picker；只改数据源 |
| FR-19 | §2.1 新表 |
| FR-20 | §2.6 迁移 + CUSTOM.md |
| FR-21 | PATCH/发送必带 session_id；校验 project/active/agent_id |
...
| AC-7 | context merge 在入队时 | 排队任务 JSON `chat_config.model` 仍为 A | 先发送再 PATCH |
...
| AC-10 | merge 保留既有键 | 夹具含 `head_sha` + `chat_config` | 入队路径 |
...
| AC-15 | 事务绑定 vs 失败补偿 | 成功后成员可见；失败仍五类空 | 发送成功/失败各一条 |
...
| AC-18 | §4.7 | 旧 session closed；新 GET 新 id；旧评论不在新 timeline | 改 `team_agent_id` |
...
| AC-22 | 472 起迁移 + CUSTOM.md | 无 FK；CONCURRENTLY；一文件一句 | 读迁移文件与台账 |
| AC-23 | POST container | GET `issue_id` 变同一 UUID；缺 session_id 不建 Issue | 无 Issue session |
| AC-24 | 三夹具 PATCH **和**发送 | Waitable+cache 成功入队；无 cache 两次 400；Blocked 拒绝 | handler+service 测试，命令见 PRD |
...
| AC-28 | sweeper 单测 | 167h59m 留 / 168h00s 留 / 168h01s 删 | `go test ... -run ChatDraftAttachment` |

无「过滤条件使 AC 不可达」：Waitable 离线发送在 AC-24① 明确允许；AC-9 明确排除 daemon 降级路径。
...
- **Workspace isolation**：所有新查询带 `workspace_id`；请求体 workspace 不得覆盖认证上下文。
...
- Bind 行锁范围小（单 session 行 + 既有 project-chat advisory lock）。
...
- 发送失败：comment 补偿删除；附件保留；不返回半成品 `issue_id`。
...
| 2 | multica | `server/internal/service/project_chat.go` | `EnsureProjectChatIssue` / `ensureContainerIssue` / `SendProjectChatMessage` / `sendProjectChatCore` | 容器创建与发送内核；Bind 后继必须复用 ensure + 行锁；GET 禁止调用 |
...
| 5 | multica | `server/pkg/db/queries/chat.sql` | `GetProjectChatSessionForCreator` | **专指** Private Ask `chat_session`；新表不得占用此名 |
| 6 | multica | `server/migrations/033_chat.up.sql` | `chat_session` 表；`agent_task_queue.issue_id` 可空 | Private Ask 扩展四列；任务可无 Issue |
| 7 | multica | `server/migrations/436_chat_session_project.up.sql` 及相关 unique | `(project_id, creator_id)` active 唯一 | FR-3 查询继续用它 |
| 8 | multica | `server/migrations/471_approval_continuation_workspace_cr_pending_unique.up.sql` | 当前最大迁移号 471 | 本 CR 从 **472** 起 |
...
| 18 | multica | `server/cmd/server/runtime_sweeper.go` | `runRuntimeSweeper` 30s 心跳 | 草稿清理用独立 1h 节流/goroutine，不进请求路径 |
...
| 21 | multica | `server/internal/handler/project.go` | `settings.team_agent_id` PATCH | 换绑后 close active session |
...
| 28 | multica | `ARCHITECTURE.md` | 不变量 1–9 | 无新队列/FK/手改 generated；注释英文 |
| 29 | multica | `server/internal/service/project_chat.go` | `MergeForwardDiscussion` / `sendProjectChatCore` | 内部 Ensure session + Bind，不改 Discussion GET |
...
迁移 472+、sqlc、`project_chat.go` handler/service、task enqueue merge、daemon claim、file 上传者门、sweeper、router、project settings 换绑挂钩、zod/client、Team Agent / Private Ask UI、locale、Go/React 测试、`CUSTOM.md`。
...
| merge-forward 无 session_id | 服务端 Ensure session，避免第二容器 |
