...
target-version: "0.32"
title: 前端：DiscussionPane session 身份 + schemas + 四语文案
...
# TASK-04 前端：DiscussionPane session 身份 + schemas + 四语文案

## 任务描述

在 multica CR worktree 重写前端 Discussion 面（SDD §3.3 前端部分、FR-19/NFR-1/NFR-2/NFR-8）：`DiscussionPane` 以 `session_id` 为唯一可写会话身份，消息/发送/附件/配置控件/实时更新全走 shared session API；旧 Discussion Issue 历史经 `legacy_issue_id` 只读渲染且不可在该流发送；响应 schema 独立 zod 定义并硬降级；消息气泡按 M486 作者字段解析展示（NULL/malformed 回退现状渲染）；新增文案 en/ja/ko/zh-Hans 对称。不重做 composer 视觉（CR-D 范围）。
...
- `packages/core/api/client.ts`：Discussion 入口/消息发送调用点走新 schema（`parseWithFallback` 复用）。
- `packages/core/types/chat.ts`：`ChatMessage` 增 `author_type?: "member" | "agent" | null; author_id?: string | null`。
- `packages/views/projects/components/discussion-pane.tsx`：以 `session_id` 为会话身份重写（拉消息走 shared 分页 API；发送走 `POST .../messages` 带 `Idempotency-Key`；附件草稿机制复用；配置控件调 `PATCH .../config`，绝不 `UpdateAgent`）；`session_id` 缺失/空/非 UUID → 硬降级只读并重试 GET，禁止拿空 id PATCH/发送；`legacy_issue_id` 只读时间线渲染且不可发送；作者解析/展示（member 缓存/agent 缓存/NULL/已移出成员四语回退）。
...
- `parity.test.ts` 对新 key 全绿；`schemas.test.ts` 覆盖 AC-17 向量。

## 接口契约

**消费（TASK-03 产出的 HTTP 契约，逐字一致）**：

- `GET /api/projects/{projectId}/discussion` → `{session_id, issue_id: null, legacy_issue_id: uuid|null, coordinator_agent_id, model, thinking_level, model_source, thinking_level_source}`。
- `GET /api/chat/sessions/{sessionId}/messages`（shared）→ 分页对象 `{messages[], limit, has_more, next_cursor?}`；`messages[]` 含可空 `author_type`/`author_id`。
- `POST /api/chat/sessions/{sessionId}/messages`（shared）→ 201 `{session_id, message_id, issue_id: null, task_id: uuid|null}`；请求头 `Idempotency-Key` 必填；错误体 `{code, error}`（403/404/409 code 见 PRD FR-18，前端据此回滚配置控件或保留草稿）。
- `PATCH /api/chat/sessions/{sessionId}/config`（shared）→ 200 配置字段（三态 PATCH：省略保持 / `null` 或 `""` 清 override / 非空设置）。

**产出**：前端类型与 schema（zod 定义见「涉及文件」），供 pane 与后续消费；不发散服务端契约。
...
3. DiscussionPane 行为核对：有 `session_id` 时不再依赖可写 `issue_id`；user 气泡作者展示（member/agent 解析，NULL/已移出成员回退）；旧 Issue 流只读（不可发送）；配置控件不调 `UpdateAgent`（AC-18）。
4. `pnpm typecheck` 相关包无新增错误（web 与 desktop 共享 `packages/views`，NFR-1）。

## 完成标志

schemas.test.ts + parity.test.ts 全绿 + pane 行为核对通过 + 四语 key 齐全，且全部已 commit 到 `requirement/CR-2026-059`（`developing` 内可被 `crctl task done` 登记的事件）。
