...
- 关键纪律：multica 代码注释一律英文（其 CLAUDE.md 硬规则）；`CUSTOM.md` 登记按 **TASK 各自负责**（FR-21/§4.9）：每个 TASK 在实施期按其当时实际结构登记本 TASK 落盘的新迁移/新文件/挂钩点（编号顺延、原因含 CR 编号与 TASK），登记纳入各 TASK 完成标志；禁止预登记猜测未来结构；`make sqlc` 生成文件不手改；状态推进只经 crctl，账本零手改。
...
| 幂等表并发收敛缺陷（FR-24） | 中 | PK 唯一索引 + `ON CONFLICT ON CONSTRAINT chat_idempotency_pkey DO NOTHING`；赢家回滚→输家可见（无死锁残留）；指纹稳定排序 | revert CR-2026-059-TASK-01/02 commit + 487–490 down |
| 跨节点断连控制不可达（AC-29） | 中 | user-scope 控制帧复用既有订阅拓扑（持有用户连接的节点必然消费 user 流）；deliverEnvelope 唯一新增分支；XADD 失败重试一次 + 请求级门禁兜底 | revert CR-2026-059-TASK-03 commit（Broadcaster 纯新增方法，既有四方法零 diff） |
| L2 配置校验最坏 30s LiveLoad 阻塞 PATCH | 低 | cache 快速路径优先；单次 PATCH 至多 1 次 LiveLoad round；owner/admin 低频 | 无需回滚；调参数即可 |
| 前端响应 schema 漂移白屏（NFR-8） | 低 | 独立 zod schema + 硬降级只读；`legacy_issue_id` 永不作可写身份 | revert CR-2026-059-TASK-04 commit |
| 移出竞态 TOCTOU（B-AUTH-2 关闭面） | 中 | `LockSubscriberWrites` 首锁 + 事务内成员复核；锁序固定 | revert CR-2026-059-TASK-02/03 commit（零数据面回滚） |
...
> 主责/关联TASK 列一律使用 canonical TASK id（`CR-2026-059-TASK-NN`，与 `tasks/_index.yml` 全等）。

| FR/关键AC | SDD交付项 | 主责/关联TASK | 验收证据 | 回滚 |
|---|---|---|---|---|
| FR-1 | §3.1 GET 重写 + §4.1 `EnsureProjectDiscussionSession`；`EnsureProjectDiscussionIssue` 解除调用；无其它 `origin_type='project_discussion'` 写入路径 | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-03） | cmd-02 | revert CR-2026-059-TASK-02/03 commit |
| FR-2 | §2.2 M482 `kind` 列 + CHECK 枚举 + 存量默认 `private` | CR-2026-059-TASK-01 | cmd-01 | 482.down（有损注记） |
| FR-3 | §2.4 M485 部分唯一索引 + §4.1 advisory 收敛 | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-01） | cmd-02 | 485.down（代码先回滚） |
| FR-4 | §3.1/§4.2 门禁一律成员资格；`creator_id` 仅审计列 | CR-2026-059-TASK-03（关联 CR-2026-059-TASK-02） | cmd-02 | revert CR-2026-059-TASK-03 commit |
| FR-5 | §2.3 M483/M484 谓词收窄 + `GetProjectChatSessionForCreator` 加 `kind='private'`（三调用点语义保持） | CR-2026-059-TASK-01（关联 CR-2026-059-TASK-03） | cmd-02 | 484.down→483.down 逆序 |
| FR-6 | §2.3 M483/M484（先建新名再删旧）+ §2.4 M485 | CR-2026-059-TASK-01 | cmd-01 | 484.down→483.down 逆序 |
| FR-7 | §2.1 M481 `agent_id` 可空 + FK `ON DELETE SET NULL`；NULL 贯穿查询盘点 | CR-2026-059-TASK-01 | cmd-01 | 481.down（先清 NULL 行） |
| FR-8 | §3.1 `base_*` 快照规则 + §4.4 L2 workspace ready-runtime 并集 authority | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-03） | cmd-03 | revert CR-2026-059-TASK-02 commit |
| FR-9 | §3.2 kind 分流；复用 `ResolveChatConfig`/`ValidateResolvedChatConfig`/`LoadChatCatalogForConfig`；无 Coordinator 校验 = §4.4 L2 | CR-2026-059-TASK-03（关联 CR-2026-059-TASK-02） | cmd-03 | revert CR-2026-059-TASK-03 commit |
| FR-10 | §4.2 普通消息只写 `chat_message`（无 task/Issue/comment） | CR-2026-059-TASK-02 | cmd-02 | revert CR-2026-059-TASK-02 commit |
| FR-11 | §4.2/§4.3 触发判定 + `CreateChatTask(issue_id=NULL)` + 快照 + 两个 409 + invocation 403 复用 | CR-2026-059-TASK-02 | cmd-02 | revert CR-2026-059-TASK-02 commit |
| FR-12 | §4.2 `writeChatCompletionOutcome` 按 `task.ChatSessionID` 写回 + 作者列 | CR-2026-059-TASK-02 | cmd-02 | revert CR-2026-059-TASK-02 commit |
| FR-13 | §3.5 `message_ids` 路径 + `RouteDiscussionToTeamAgent` 触发源适配（内核 zero_diff） | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-03） | cmd-02 | revert CR-2026-059-TASK-02/03 commit |
| FR-14 | 复用 CR-2026-056 草稿契约（五空上传、上传者门、168h sweeper 谓词不改） | CR-2026-059-TASK-03（关联 CR-2026-059-TASK-02） | cmd-02 | 复用既有，无新增回滚面 |
| FR-15 | §4.2 同事务绑定 `BindDraftAttachmentsToChatMessage` + 失败零残留 + 草稿保留重试 | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-01） | cmd-02 | revert CR-2026-059-TASK-02 commit |
...
| FR-24 | §2.6 M487–490（建表/唯一索引/挂 PK/辅助索引）+ §4.6 收敛算法 + §3.4/§3.5 缺头/冲突错误 | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-01/CR-2026-059-TASK-03） | cmd-02 | 487–490 down 逆序 + revert |
| FR-25 | §3.1/§3.2/§3.4 成员门禁 + §4.7 实时拒绝/退订 + §4.8 附件 404 + 草稿仅上传者 | CR-2026-059-TASK-03（关联 CR-2026-059-TASK-02） | cmd-02 | revert CR-2026-059-TASK-03 commit |
| FR-26 | §4.5 写权威/投影/读规则/竞态/归档/hard-delete 全表 | CR-2026-059-TASK-02（关联 CR-2026-059-TASK-03） | cmd-02 | revert CR-2026-059-TASK-02 commit |

**双向集合断言（B-DP-01 修复；已在本修订落盘前实测）**：令 A = 本表「主责/关联TASK」列出现的全部 `CR-2026-059-TASK-NN`（canonical 解析）集合，B = `tasks/_index.yml` 的 `tasks[].id` 集合。回修后必须满足 A = B（表内引用不到账本、或账本存在表内不引用 → 即失败，非 WARN）。当前 A = B = {`CR-2026-059-TASK-01`, `CR-2026-059-TASK-02`, `CR-2026-059-TASK-03`, `CR-2026-059-TASK-04`}，与 §7 AC 矩阵的 owner 列一致；writeback 期 `writeback-traceability.mjs` 以完整 id 正则解析本列，短名 `TASK-NN` 一律禁止。
...
> 关键 AC（影响主路径验收可达性）每行唯一 TASK owner（canonical id），验收证据为稳定 `cmd-NN`。

| AC/业务闭环 | SDD 落点 | TASK owner | 验收证据 |
|---|---|---|---|
| AC-1 打开不建 Issue | §3.1/§4.1 | CR-2026-059-TASK-02 | cmd-02 |
| AC-2 四类操作零工作 Issue | §4.2/§4.5 | CR-2026-059-TASK-02 | cmd-02 |
...
| AC-10 非法配置 PATCH/入队 400 | §4.4/§4.2 | CR-2026-059-TASK-03 | cmd-03 |
| AC-11 未绑定 Coordinator 409 | §4.3/§4.2 | CR-2026-059-TASK-02 | cmd-02 |
| AC-12 草稿仅上传者可见 | §4.8 | CR-2026-059-TASK-03 | cmd-02 |
| AC-13 发送成功原子绑定 | §4.2 | CR-2026-059-TASK-02 | cmd-02 |
...
| AC-25 merge-forward 互斥/跨源/去重 | §3.5 | CR-2026-059-TASK-03 | cmd-02 |
