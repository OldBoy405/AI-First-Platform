...
| `server/internal/daemon/execenv/crguard_config.go` | 改（新增**可空入参**；在既有 claude 分支的单写入点内合成 OutputGuard hooks；入参为空时行为逐字不变） | §4.9 A9 第 3/5 步 |
...
3. **nil 入参不变量**：`prepareCRGuard` 在 OutputGuard 入参为空（或 Tools Release 根未配置）时，写出内容与改造前**逐字相同**（含 JSON 缩进与末尾换行）；新增测试必须逐字断言这一点。
...
7. **测试命名约定（硬）**：新增测试函数一律以 `TestOutputGuard` 前缀命名（`cmd-06` 的 `-run TestOutputGuard` 与 `cmd-07` 的存在性守卫按此前缀取值），且**至少 4 个**，分别覆盖：claude 单写入点合成（既有段在前 + OutputGuard 段在后 + 单次写）/ 目标文件已存在不 clobber / 入参为 nil 时既有输出逐字不变 / 未配置 Tools Release 根时整段挂载跳过。
...
3. `crctl git diff --name-only <multica base> --cwd <multica worktree>` 的路径集 ⊆ {`CUSTOM.md`, `crguard_config.go`, `outputguard_config.go`, `outputguard_config_test.go`}，且四个文件全部在集合内；`tool_output_preview.go` / `governance/runner.go` / `pkg/agent/pi.go` / `runtime_config.go` 零 diff。
...
- **不**包含：为其它 Runtime 新开 daemon 写点、改 `execenv.go` 的调用点以外结构、任何平台 Prompt / 生成物改动（R-15 由 owner 部署窗口承担）、任何共享服务启停。
...
// server/internal/daemon/execenv/crguard_config.go（既有文件，新增可空入参）
func prepareCRGuard(envRoot, workDir, provider, agentCaller string, logger *slog.Logger, og *outputGuardMount) (CRGuardResult, error)
// og == nil ⇒ 既有行为逐字不变（含 {workDir}/.claude/settings.json 的 JSON 缩进与末尾换行）
// og != nil ∧ provider == "claude" ⇒ 在同一个 settings 对象内合成：既有 hooks 段在前、OutputGuard 段在后，单次写入
// og != nil ∧ provider != "claude" ⇒ 不写任何 hooks 配置（provider == 比较在函数内恰出现 1 次）
// 目标文件已存在 ⇒ 不写/不合并/不覆盖，沿用既有告警句，挂载记为未完成
```

```text
CUSTOM.md《代码改动明细》新增稳定 ID #95（不新造列、不改表头）
  位置：server/internal/daemon/execenv/crguard_config.go（新入参，// AIFIRST: 标记）+ outputguard_config.go（新文件）
...
- `dep-4`：`prepareCRGuard` 的既有形态与"存在即跳过并告警"语义（只读参照，本 CR 只加可空入参）。
