...
- **逃生**：离线（adapters-contract）——命令首行 `# output-guard: full reason=<一句话>` ⇒ 无决策（放行原调用，跳过 OutputGuard 封顶）；四条负向（git 白名单 / 受控路径 / 审批 / 账本写入）逐条断言逃生阀不产生 deny。真实会话冒烟：未执行。
...
- **逃生**：离线（adapters-contract）——逃生阀合法 ⇒ 无决策（不写 `permissionDecision`、不 deny）；四条负向逐条断言。真实会话冒烟：未执行。
...
- **逃生**：离线（adapters-contract）——逃生阀合法 ⇒ 无决策；四条负向逐条断言。真实会话冒烟：未执行。
