...
updated: "2026-08-18T20:40:00+08:00"
---
...
lib/workspace-transactions.mjs   ← 本 CR 唯一实现落点
   ...
   └─ reconcileLocalTrunks（新增私有 helper，mergeCr 调用）
   ↓ import（既有）
...
注册（缺口 A）：
  ensureRepoWorkspace 首次 classify = missing
    → git fetch origin（失败 → WORKSPACE_TRUNK_UNAVAILABLE，零 CR 资源写入）
...
- `missing`：先 `git fetch origin`，再重新分类，按 1.3 流程创建或阻断。
...
- 错误语义：**绝不抛出**。所有 git 失败、状态不符均转换为行结果（PRD FR-10 的 exit 0 前提）。`gitRun` 判定、`git merge --ff-only` 用 try/catch 捕获 `TxError` 转 `failed/ff-only-failed`。
...
- 事务重跑（wasComplete=true）同样执行 helper：幂等，结果通常全 `unchanged`，不产生新副作用（PRD NFR-2）。
...
- `classifyRepoWorkspace` 在 worktree 缺失时提前返回，故 fetch 后重新分类只会出现 `branch-only`（并发建分支）、`remote-only`、`missing` 三类；前两者走既有恢复路径，其余（不可达但防御性）硬阻断。
...
for repo in ctx.repositories:
  ...
  fr = gitRun(rootPath, ['fetch', 'origin'])
  if fr.status != 0:                                              # 判据 3
...
安全属性：每一步都只读 + 条件推进；唯一写命令是 `git merge --ff-only <捕获 SHA>`；无 reset/stash/rebase/普通 merge/checkout；失败即本仓终止，不影响其他仓与远端结果。
...
helper 在 merge 目录锁内执行（return 在 `finally` 释放锁之前）：与既有 merge 事务互斥，无需新锁；不写 journal，进程中断后该同步即不补偿（PRD FR-10，用户可原生 `git pull --ff-only`）。

## 5. 技术选型与替代方案

| 决策 | 选择 | 替代方案与否决理由 |
|---|---|---|
| 注册基点 | `fetch` + 重新分类 + `git branch <cr> refs/remotes/origin/{trunk}` | register journal 记录 `base-sha/source`（PRD §4.3 否决：本地 ref 已是基点权威事实，journal 复制 Git 事实且跨仓无共享 SHA） |
...
| FR | 实现位置 | 方式 |
|---|---|---|
| FR-1 | `workspace-transactions.mjs#ensureRepoWorkspace` `case 'missing'` | fetch → `classifyRepoWorkspace` 重新分类；healthy/branch-only 路径零改动（天然不 fetch） |
...
| FR-5 | 同上 | 重新分类结果非 remote-only/missing → 复用 `WORKSPACE_ENSURE_BLOCKED` 硬阻断 |
...
- **现场零破坏**（PRD NFR-1）：两个修改点合计的写命令仅 `git fetch origin`、`git branch`（新分支/--track）、`git worktree add`（既有 create 闭包）、`git merge --ff-only <捕获 SHA>`。无 reset/stash/rebase/普通 merge/强制 checkout/分支删除；无回滚或补偿事务。
...
- **审计**：新增错误码走既有 `fail/TxError` → `.crctl/audit.log` 路径；`localTrunkSync` 刻意不写 audit/journal（PRD NFR-5 唯一观察面 = merge 输出）。

### 7.2 性能

- 新增 git 调用：注册 missing 仓 +1 fetch、+1 重新分类（4 次只读 git）；merge helper 每仓 ≤6 次 git（1 fetch + ≤5 只读/写）。
...
| 进程在 save('complete') 后、helper 前退出 | 同步不补偿；重跑 merge 时 helper 再次执行（幂等） |
...
4. **trunk 缺失**：bare origin `update-ref -d refs/heads/master` → fetch 成功后抛 `WORKSPACE_TRUNK_UNAVAILABLE`（AC-3）。
...
2. **表驱动单元测试**（直接调 `reconcileLocalTrunks`，用 register fixture 的 bare 三仓构造状态）：wrong-branch（checkout 其他分支）、dirty（未提交文件）、diverged（本地前进且 origin 也前进）、fetch-failed（bogus url）、trunk-unavailable（bare 删 master）、unchanged（已同步）→ 断言 status/reason/before/remote/after 按 §2.2 表取值（AC-6）。
3. **ff-only-failed 集成**：`CRCTL_FAULT_POINT=local-sync-ff-only-failed` 跑完整 merge → exit 0、`phase=complete`、kb 行 `failed/ff-only-failed`、远端 master 已含 merge commit（AC-6/7）。
