...
updated: 2026-08-13T09:58:32+08:00
---
...
```json
{
  ...
  "graphDigest": "<64-hex>",
  "inputDigest": "sha256(cr+graphDigest)",
  "sideEffects": [],
  "lastError": null,
  "checkpoint": {
    "repositories": [
...
journal 可记录本机恢复细节，但这些字段不进入 batch-id 或公开 snapshot。graph 在首个 source commit/push 后变化返回 `GRAPH_CHANGED_DURING_TRANSACTION`。metadata confirmed 后把 phase 写为 complete；确认 authority 后可删除该 completed checkpoint tx 目录。若进程死在 complete 后、删除前，下一次非 no-op 调用先验证该 batch 仍为 authority，再清理旧 complete journal 并创建新事务。
...
- no-op 不发事件；事务中间逐仓 push 不发事件，避免 projected_commit 指向半批次；
- outbox 失败只写 audit，不回滚 Git authority，也不改变公共成功字段。

Multica 不改代码；现有 `TestCheckpointFillsProjectedCommitForEmbedded` 保持通过，tools 侧增加“完整批次一次事件/no-op 零事件”契约测试。

# 4. 关键算法与流程

## 4.1 全仓 preflight 与敏感文件检查

在任何 `git add/commit/push` 和 journal 创建之前：

1. `resolveRepositories(workspace)`，验证 active repo graph，要求 workspace 对应当前 CR；
2. 对每仓定位 `{repo.worktreePath}/{cr}`，验证真实路径受 containment、worktree 已注册、当前分支恰为 `requirement/{cr}`；
3. 从 KB `cr.md` 读取 status；`cmdCheckpoint` 复用状态机事实源，只允许非终态；
4. fetch 每仓 `origin requirement/{cr}` 并记录精确 remote HEAD（允许首次不存在）；
5. 用 `git status --porcelain=v1 -z --untracked-files=all` 收集本轮新增/修改普通文件的 Git POSIX path；
6. 先执行固定路径规则，再读取这些普通文件检查 `-----BEGIN ... PRIVATE KEY-----` 头；
7. 任一仓失败则全仓零 add/commit/push，敏感命中统一 `CHECKPOINT_SENSITIVE_PATH`。

固定路径规则与 PRD FR-02 完全一致。路径比较大小写精确；symlink/目录/已删除文件不做内容读取，仍由 Git 路径与 repo containment 约束。扫描复杂度为本轮变化普通文件总字节数，不引入 scanner 依赖或例外配置。

## 4.2 journal 前 no-op

仅当以下条件全部成立才返回 no-op：

- 现有 `latest-checkpoint` schema 合法，graphDigest 与当前 graph 一致；
- 所有 worktree 无未忽略变化；
...
git diff --cached --quiet
  exit 0 -> clean，sourceSha = HEAD
  exit 1 -> git commit --no-gpg-sign --file=-，sourceSha = new HEAD
  其他   -> TX_GIT_FAILED
复核 worktree/index clean
journal repo.phase = committed-local
```

commit message 保持 `wip: {cr} {repo} checkpoint{messageSuffix}`。journal 在每个本地 commit 后 durable 保存；崩溃重跑时若 HEAD 已等于 sourceSha 则不重复 commit。
...
1. 将当前 KB HEAD 固定为 `kbSourceSha`；确认 KB remote 等于它或是它的祖先，其他关系按 exact-head 错误阻断；
2. 以所有 repo source facts 生成 batch-id 与完整 snapshot；
3. `editLatestCheckpoint()` 生成 `_backlog.yml` after image，并用既有 one-entry `applyWriteSet()` 绑定 before/after hash durable 应用；
4. 只 stage `change-requests/_backlog.yml`，复核 staged set 恰为该文件；
5. commit message：`[cr] checkpoint {cr} batch {batchId}`，并带 `AI-First-Op: checkpoint` / `AI-First-Tx` / `AI-First-CR` trailer；
6. 复核 `metadataCommit^ == kbSourceSha`；
7. 以 KB remoteBefore 做 lease push；push 后精确确认 remote HEAD == metadataCommit，且直接父提交 == kbSourceSha；
8. journal phase=complete，返回完整结果；随后 best-effort 清理 completed journal。

只有其他 repo 变化时，KB 不造空 source commit；`kbSourceSha` 可以是上一轮 metadata HEAD，新 metadata commit 直接接在其上，不形成无业务变化的 M1→M2→M3 自循环。

## 4.6 中断恢复

同一 `crctl checkpoint` 入口按 journal repo phase 恢复：

- `prepared`：补 source commit；
- `committed-local`：按 remote facts push/confirm；
- `pushed`：先观察 remote，响应丢失时转 confirmed；
- `confirmed`：跳过已完成 repo；
- `metadata-committed`：验证 commit、直接父与 snapshot，不重复 commit；
- `metadata-pushed`：确认 remote authority；
- `complete`：返回同一 batch，或在发现新内容前清理旧 complete journal并开始新事务。

恢复不 reset/checkout/rebase/cherry-pick/force，不补偿 revert。journal 中任一记录与 Git 第三值冲突即硬失败。
...
| FR | 技术方案 | 主要验证 |
|---|---|---|
| FR-01 | `cmdCheckpoint` + `checkpointCr`；resolver/状态守卫；固定输出 | CLI 黑盒、worktree/branch mismatch、tracked/untracked/ignored fixture |
...
| FR-05 | checkpoint journal + exact-head classifier + lease publish | kill/restart、response lost、advanced/diverged/history rewrite |
...
1. worktree/branch mismatch：全仓零 add/commit/push；
2. 敏感路径/私钥头：index 原样、全仓零 commit/push；例外文件不误拦；
...
6. 第二仓 push 后进程退出，重跑不重复已 confirmed push；
7. push 响应丢失，重跑观察 remote 收敛；
8. remote ancestor source：lease fast-forward 后精确相等；
9. remote advanced/diverged/history-rewritten：对应错误且零 metadata；
10. metadata commit/push 前后故障与响应丢失：单 metadata commit，direct-parent 正确；
11. 只有非 KB repo 变化：KB source 为上一 HEAD，不造空 source commit；
12. CRLF backlog、Windows path、未知 fault point 硬失败；
13. 完整批次只发一次 checkpoint outbox，no-op 零事件；
14. T05 静态扫描证明 active Skill/Pipeline/README/help 无旧 checkpoint-add 算法/字段。
...
| checkpoint outbox 丢失 | CLI 层保留 metadata-confirmed batch 事件；Multica 现有消费者契约测试保持 |
