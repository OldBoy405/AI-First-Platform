...
- Agent 只保留路由、职责判断和 Skill 委派；不管理共享服务生命周期，不等待下游任务。
- implement-code 只做一次环境检查，任务范围内可修正时最多重跑一次；超出权限时返回 `ENVIRONMENT_MISMATCH`。
...
3. 文档内引用的既有命令、Skill 和 Pipeline 名称均能在 tools worktree 中定位。
