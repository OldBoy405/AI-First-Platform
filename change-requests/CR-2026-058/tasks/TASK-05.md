...
输入条件：TASK-01（guard 向量）、TASK-02（plan 向量）已在 crctl.test.mjs 落盘且全绿。
...
2. **AC-4 静态断言**（读文件断言，不引入第二套测试框架）：
   - `writeback-tx.test.mjs` 不再断言「cr.md=unassigned + 输入真实版本 → WRITEBACK_VERSION_UNASSIGNED」：grep 该文件确认无「unassigned 正向拒绝」断言（TASK-04 改写事实的静态核对）；
...
2. 静态核对：本 TASK 新增断言覆盖 AC-4 三项事实（writeback-tx 改写、错误码收敛、零观察点保持——零观察点在 cmd-02 拒绝路径保持由 TASK-04 断言，此处以 grep 断言 writeback-tx.test.mjs 保留 `snapshotSixPoints` 类调用）。
...
- 消费：TASK-01/02 落盘的 crctl.test.mjs 向量与 `planVersionRefill` 导出行为；`resolveRepositories(kb)`。
