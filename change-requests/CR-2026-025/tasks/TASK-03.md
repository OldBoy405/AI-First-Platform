...
- **cmdReviewRecord 重构**（SDD §4.4a）：全部前置校验（stage 映射/前置态/payload schema/exhausted/trace 结构/CR-ID 一致性）在任何写入之前；`recordedAt = nowIso()` 一次生成三账本共用；构造 annotation/trace/loop 三份文本后交既有 `casWriteMulti`（annotation 首建 `expectedHash=null`）；未带 `--bump-attempt` 时不写 review-loop.yml，投影取其当前轮次；review-loop.yml 缺失的非 bump 场景按 attempt=1 处理（TD-SUG-3）。
- **attempts 历史合并**（§4.4a 2b，TD-BL-1/TD-BL-4）：`stageNode = parseYaml(traceText)?.reviews?.[stage]`；`stageNode === undefined` → 首写合法，`oldAttempts = []`；存在时读 `review-loop.attempts`（键缺失 → `[]`，形状不合 → `TRACE_SHAPE`）；bump 追加新条目（重号 → `TRACE_SHAPE`），非 bump 按 current-attempt 整条替换或追加。历史数据源唯一 = trace 现有投影，禁从 review-loop.yml/annotation 臆造 result/blocker-count。
