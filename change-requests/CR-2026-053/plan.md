...
```
M1 (TASK-01/02/03) ──────────────┐
                                  ├──→ M3 (TASK-04 需 01/03/05；TASK-07/08/11 需 05) ──→ M4 (TASK-09 需 05/06/07/08；TASK-10 需全部前序)
M2 (TASK-05/06) ──────────────────┘
```
...
- TASK-04 依赖 TASK-01 + TASK-03 + TASK-05（**消费 TASK-05 的绑定接口**，端到端验证需接口先就绪）；
- TASK-07/08 依赖 TASK-05；TASK-11（存量修复）依赖 TASK-05；
- TASK-09（CUSTOM.md 台账）依赖 TASK-05/06/07/08；
...
Track A (tools 仓):
  ...
  CR-2026-053-TASK-04: 修改 review Skill 绑定前置步骤                                    → [01, 03, 05]
...
Track B (Multica 仓):
  ...
  CR-2026-053-TASK-11: 存量 CR-2026-051/052 修复                                        → [05]
...
- TASK-04 依赖 TASK-01 + TASK-03 + TASK-05
- TASK-07/08 依赖 TASK-05；TASK-11 依赖 TASK-05
...
| review Skill 绑定失败静默降级 | 高 | **硬失败不降级**：有 mat_ context 且绑定失败时，直接 `crctl review-record --abort` 或中止，零绑定写入、不写 canonical review（FR-B7/NFR-6）。**分仓回滚**：tools 仓 revert 四个 review SKILL.md 提交即移除前置步骤。**部分发布处置**：tools 仓单独合入（multica 接口未上线）时绑定步骤硬失败于 5xx，review 流程被阻塞——禁止 tools 仓先于 multica 合入。 |
...
- [ ] tools 仓：`node ../tools/skills/shared/crctl/scripts/check-skill-matrix.mjs` 零报错
- [ ] tools 仓：`node ../tools/skills/shared/crctl/scripts/check-agents-contract.mjs` 零报错
- [ ] tools 仓：`node ../tools/skills/shared/crctl/scripts/lint-prompts.mjs` 零报错
- [ ] tools 仓：`node ../tools/pipeline-templates/emit-registry.mjs --verify` 通过（验证 Track A 改造后 registry 一致性）
...
- [ ] Multica 仓：`pnpm test packages/views/projects/components/` 覆盖 AC-C1~C6
