...
status: pending
estimate: 2h
depends-on: [CR-2026-053-TASK-01, CR-2026-053-TASK-03]
created: 2026-08-28T11:20:00+08:00
---

## 任务描述

在四个 review Skill 的 `review-record` 之前插入同一绑定前置步骤：

```text
若当前运行具有 Multica task-scoped context（存在 mat_ token 注入的 task 上下文）：
    result = bind-current-task-to-cr(CR-ID)
    if result 失败:
        → 按技术失败中止，不写 canonical review
否则：
    → 视为普通本地/非 Multica 执行，跳过绑定，继续现有 review-record 行为
```
...
- 技术失败硬失败，不静默降级
- 无 Multica task context 时跳过绑定

## 验收条件

1. 四个 Skill 均含同一绑定前置步骤
2. 绑定步骤位置正确（在 review-record 之前）
3. 技术失败处理逻辑正确

## 完成标志

- 四个 Skill 文件修改已 commit
- 验证 Track B 绑定接口可用后，端到端测试通过
