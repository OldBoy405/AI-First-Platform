...
updated: "2026-08-10T16:39:11+08:00"
---
...
- **FR-2（workspace 双根语义）**：Operational Workspace 负责 CR 阶段文件读写；Installation Workspace 负责 Tools Root 路径基准。普通 checkout 两者相同；knowledge-base linked worktree 通过 Git common-dir 找主 checkout。不得改写 worktree graph、创建 symlink/junction或新增 `--tools-root`。tools/multica worktree 必须显式使用 requirement-register 已输出的 `knowledge_base_worktree` 作为 `--workspace`，不得按分支名、CR-ID 或目录扫描猜测。
...
- **FR-5（active 执行入口统一）**：active crctl/writeback Skill、feature-writeback Pipeline、Claude Code/Qoder/Cursor/Codex/CI 模板与安装说明、tools 当前入口文档中的实际命令改用 `{TOOLS_ROOT}/skills/...` 逻辑路径并明确来源。动态调用方运行时解析；静态模板安装时物化。仓库不得提交含本机绝对路径的物化 settings；历史分析、审查报告、生成 HTML、OpenWiki、inactive/old 内容不做全仓替换。
...
- **FR-9（最小回归验证）**：扩展现有 `crctl.test.mjs`：公共 workspace fixture 显式绑定测试 tools 包；表驱动覆盖相对/绝对路径、空壳目录、缺配置、无效路径、四标志缺失；增加一个 Git linked-worktree 黑盒场景；复用现有 cr-init metadata 测试。active 文档/模板使用定向检索验证，不新增 resolver 测试包、mock filesystem、IDE E2E 或跨平台 Adapter matrix。
...
- **AC-4（FR-2）**：从 Installation Workspace 根目录及任意子目录调用，解析结果相同；从 knowledge-base linked worktree 根目录及子目录调用，CR 文件来自该 worktree而 Tools Root 相对于主 checkout 解析。
- **AC-5（FR-2）**：从 tools/multica worktree 显式传 `--workspace <knowledge_base_worktree>` 可用；不传时不承诺反推对应 CR，且实现中不存在分支名、CR-ID 或 `.rayai-worktrees` 扫描逻辑。
- **AC-6（FR-4）**：status/gate/advance 等涉及配置的命令报告的 state machine、Pipeline、gates、默认 rules 均位于同一 Tools Root；执行脚本来自另一 checkout 时不得影响默认配置来源。
- **AC-7（FR-4）**：设置有效 `CRCTL_RULES_PATH` 时仍使用显式 rules；未设置时使用 Tools Root rules；无新增 gates/rules 覆盖环境变量。
- **AC-8（FR-4）**：同一 CLI 调用中 Tools Root 配置只读取/验证一次；`crctl help` 在无 workspace 时仍成功。
- **AC-9（FR-5）**：active Skill/Pipeline/Adapter/CI 当前命令不再把 `<workspace>/tools/` 当固定事实；模板使用 `{TOOLS_ROOT}` 且安装说明说明其来自 `workspace.tools_package_path`。
...
- **AC-15（FR-9）**：`node --test skills/shared/crctl/scripts/test/crctl.test.mjs` 全绿，包含路径表驱动与 linked-worktree 场景；没有新增独立 resolver/Adapter 测试框架。
- **AC-16（NFR-4）**：现有状态机、gate、approve、账本、cr-init、worktree-path 相关回归测试全绿；状态机声明数与转换语义无变化。
- **AC-17（全局）**：`git diff --check`、Pipeline JSON parse、`lint-prompts.mjs --mode enforce`、active surface 定向路径检索全部通过。
