...
updated: 2026-08-17T00:02:54+08:00
---

# 1. 交付里程碑

基线：`tools@custom/main` commit `8f530589`；代码仓为 tools（`.rayai-worktrees/tools/requirement/CR-2026-044`），multica 不参与代码改动。
...
| M5 采用面 | TASK-05：`workspace inspect` authority path、三条 Pipeline checkpoint 合同、六类 Skill 文本与结构测试 | TASK-03、TASK-04 | 6h |
...
TASK-01 (红测试)
  ├─→ TASK-02 (TTY y|yes)          ─┐
  ├─→ TASK-03 (release 本地化) ────┼─→ TASK-05 (Pipeline/Skill 采用)
  │         └─→ TASK-04 (merge preflight) ─┘
  └─────────────────────────────────────→ TASK-06 (文档 + 全量回归)
```

- TASK-02、TASK-03 相互独立，可并行；同仓测试文件冲突时串行。
- TASK-04 复用 TASK-03 的本地 verifier 作为 merge 共同前置。
- TASK-05 消费 TASK-03 的 inspect 契约与 TASK-04 的 checkpoint 完成条件。
- TASK-06 只做文档与回归，不改行为。
...
- 发布前 checklist：
  ...
  2. `crctl.test.mjs`、`checkpoint-tx.test.mjs`、`merge-tx.test.mjs`、`writeback-tx.test.mjs`、`archive-tx.test.mjs`、`pipeline-structure.test.mjs` 全绿；
