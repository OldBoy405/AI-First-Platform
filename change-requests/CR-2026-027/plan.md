...
| M5 review-record 输出深化 | 返回 files/attempt/route（按 stage 真值表）/repairTarget；pass 时 repairTarget=null；四个 review Skill 删除 traceability 二次读取 | tools `crctl.mjs`、`crctl.test.mjs`、`skills/{requirement,develop}/review-*.SKILL.md` | 1 人天 |
...
M1（bootstrap + Phase 0 文档统一）——tools worktree 是一切 tools 改动的落点
  ...
  ├─> M4（archive 原子化 + 终态查询）
  └─> M5（review-record 深化）    # 依赖 M4 的终态 resolver 无、与 M2 无耦合
