...
title: 终态只读 resolver（status/next fallback）+ inbox-emit 空 to 校验
slug: terminal-resolver-inbox-emit-guard
status: pending
estimate: 6h
...
归档后 status/next 不再返回 `CR_STATUS_NOT_FOUND`：新增仅供 status/next 使用的终态只读 resolver；同时修正普通 `inbox-emit` 的空收件人校验（与 Skill 契约对齐）。
...
4. `cmdInboxEmit`：`--to` 缺失、解析后非列表、去重后为空 → `BAD_ARGS`，不写无收件人 notify-log（B-13）
...
crctl.test.mjs 终态查询 + inbox-emit 校验用例全绿；既有 status/next/inbox-emit 用例不回归。

## 接口契约

- 消费：TASK-01 产出的 tools worktree；TASK-06 的 archive-move（产生 history 终态条目）
- 产出：`resolveTerminalCrState(ws, cr)`（错误码 `HISTORY_DUPLICATE_ENTRY`/`HISTORY_FINAL_STATUS_MISSING`/`CR_LOCATION_CONFLICT`）、`cmdInboxEmit` 空 to 校验；TASK-10 的 AC-16/AC-17 验收基于本产出
