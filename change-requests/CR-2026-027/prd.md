...
- **FR-10（归档残留幽灵条目迁移清理）**：新增入库的一次性版本化迁移脚本（位于 `../tools/skills/shared/scripts/`，与 writeback 脚本同层），删除 `_backlog.yml` 尾部缺 `- id:` 行的 CR-2026-024 幽灵条目块；删除依据为 `_history.yml` 中存在 CR-2026-024 完整归档条目；运行后 CR-2026-017 条目字段恢复完整；脚本幂等（已清理时返回 `already-clean`），再次运行不得重复修改；脚本行为纳入 crctl 或对应测试覆盖（B-12 场景回归）。
...
- **AC-9**（FR-8）：gate/签名/证据预检失败时零文件写入；CAS 冲突时 approval.yml 与 cr.md 均不写；commit 失败时两文件共同保留在工作区且不发 status outbox。
- **AC-10**（FR-8）：拒绝路径不写批准段，仍走现有合法 reject 转换（如 `approve-requirement:reject -> write-requirement-prd`）。
