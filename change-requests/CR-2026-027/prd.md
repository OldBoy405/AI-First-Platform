...
- **AC-10**（FR-8）：拒绝路径不写批准段，仍走现有合法 reject 转换（如 `approve-requirement:reject -> write-requirement-prd`）。**AC-10b（代码评审二轮 b10）**：`approve --resign` 非交互式调用一律 `APPROVAL_REQUIRES_HUMAN`（无旁路）；无既有审批段 → `RESIGN_NO_PRIOR_APPROVAL`；digest 已一致 → 幂等 no-op；迁移后 gate 复绿且 resign 审计子块落盘。
