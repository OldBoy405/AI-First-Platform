...
1. **AC-2／AC-3 的场景与观测窗口**（SDD §4.4，逐条照做，不扩面）：
   ...
   - **对账规则（采集 run 执行）**：节点发出的数组必须与 `multica agent tasks <agentId> --output json` 中该 run 的 `tool_calls.calls[].tool` 序列**同序前缀相等**；平台尾部差集 ≤ 2 条，且只能是「读取 `PI_SESSION_FILE` 的取数调用」与其后的「发布回报评论的调用」——出现任何其他差异即对账不成立，硬失败（**禁止**以平台名单臆造或补写 `command` 文本，那会把节点自证面扩大成臆造面）。文本面（`command`）仍是节点自证项，平台面独立核验的是 `seq`／`tool` 与 run 存在性。
   - **节点边界（下发时必须逐条写入节点指令）**：① 取数调用是该节点结束前的**最后一次**工具调用，其后只允许发布回报评论（否则尾部差集不可判）；② 回报必须 mention `dev-agent`（否则下一跳采集 run 不被唤醒）；③ 只读边界不变（不产出 verdict、不写 `review-annotations/**`、不调用 `crctl`、不改仓库文件）。
