...
| cmd-06 | 可达 | **exit 1 / 1 failure**（`evidence/pi-source.json` 未落地，重跑实测 0.4 s）；**Pi 侧基础设施本节点已实跑就位**：clone 21.5 s、`npm ci` 2 m 14 s、hydrate 4.9 s、`build:offline` 9.7 s、探针复跑 2.7 s | 变更后须 failures = 0（变更文件集 + 用例复跑 + 日志 sha256 + 安装包负向） |
