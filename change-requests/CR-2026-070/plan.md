...
- **依赖序固定为 `{TASK-01, TASK-02} → TASK-03`；`{TASK-01, TASK-02} → TASK-04`（TASK-04 不依赖 TASK-03）**：与本节 ASCII 图、§9 预分配表、`tasks/_index.yml#depends-on` 逐条一致。TASK-01 与 TASK-02 落在不同仓／不同目录（multica worktree vs 机器本地 Pi 检出），**互不共用文件**、可并行；TASK-03 的台账行必须按两者的**最终**文件清单与原因追溯填写；TASK-04 的证据记录引用两者的交付事实（`pi-source.json` 记录 TASK-02 的检出形态（detached HEAD @ `d981de12`）／commit／file 集，`fr1-smoke.json` 记录 TASK-01 落地后运行环境的 run 事实）。
...
| RU2 | TASK-02 在 Pi 检出（detached HEAD @ `d981de12`，**无分支**，DEC-3）上的提交（`src/core/tools/bash.ts` + `test/bash-default-timeout.test.ts`） | 在检出内 revert 该提交（检出非 CR 仓，不进 checkpoint 面） | 无（未发布、未安装；运行环境替换发生在 CR 合并之后） |
...
| R-10 | 长任务夹具（`process.execPath -e` 的 `setInterval` 空转进程，故意永不退出）在红灯迭代／断言失败／异常路径上**残留** | 清理不得只依赖被测的 `killProcessTree()` 在超时分支上生效：TASK-02 §3.5 要求夹具**自带兜底清理**（`finally`／`afterEach` 内对父与后代 PID 做有界 kill，独立于被测代码路径）。残留后果 = 作者 run 的进程树／管道不闭合 ⇒ 该 run 不结算、后续委派排队（本 CR 来源 Issue AIFI-33 已有同类实测：run `01a0b26f-185a-704b-8e08-77137a9aab33` 留下两个该形态进程，靠外部 kill 后才结算）——而 TASK-02 §5 的完成标志依赖作者 run 正常跑完并登记 `crctl task done`，即验收路径自身被拖住。出口 = 兜底 kill ＋ 有界轮询断言（用例 2 的 ≤10 s 轮询） |
...
⑥a **平台查询与用例复跑的 `maxBuffer`（回修轮实测修正）**：Node `spawnSync` 默认 `maxBuffer` = 1 MiB，超限时杀子进程并以 `error.code = ENOBUFS`、`status = null` 返回——`cmd-05` 的 `multica agent tasks <agentId> --output json` 在 run 数较多的 Agent 上实测输出 **1,065,083 B > 1 MiB**（agent `2ed1a9de-4c8e-4b78-bfb1-055af99c6681`（`quality-reviewer-agent`），2026-09-18 实测），未设 `maxBuffer` 会得到 `FAIL multica agent tasks … exit=null（平台 run 记录不可判）` 的硬失败（该 Agent 正是 TASK-04 三条 run 的预期载体）。故 `cmd-05` 的平台查询与 `cmd-06` 的 vitest 活体复跑两处 spawn 均已显式设 `maxBuffer: 67108864`（64 MiB）：只是提高截断上界，**不放宽任何判据**（退出码、字段与断言名仍逐条硬判）。
...
| cmd-03 | 可达 | **exit 1 / 8 failures**：规则四子句缺失 4 项、缺常量符号 `skillsRoutingRule` 1 项、测试文件未引用常量 1 项、`CUSTOM.md` 缺 CR-2026-070 台账行 1 项；`scanned=1492 copyFaceHits=0`（零复制面本已干净，**无一项来自 zero_diff 面**） | **时点判据（随 TASK-01／TASK-03 分批）**：TASK-01 完成时点须 failures = **1**，且唯一失败项为 `FAIL CUSTOM.md 缺 CR-2026-070 台账行`（TASK-03 交付面，见 TASK-01 §4 第 3 条／§5 第 2 条）；TASK-01＋TASK-03 齐备后（本 CR 变更全部落地）须 failures = 0（八项逐条对应 TASK-01 的 4 个交付面与 TASK-03 的台账面） |
...
| cmd-05 | 可达 | **exit 1 / 1 failure**（按 §6.2 表内字面 args，**含 B-3 新增的两条 AC-4 断言**）：`evidence/fr1-smoke.json` 未落地（本节点无该证据文件，属**显式红**而非假绿）；新增断言已用合成证据文件逐条验证：受控文件集缺 `review-annotations/*.yml` 键→红（定向 FAIL）、`reviewAnnotationsNewFiles` 缺失或非空→红（定向 FAIL），补齐后二者转绿 | 变更后须 failures = 0（三条 run 记录 + 独立重算零命中 + 受控文件集含 `review-annotations/*.yml` 且前后 sha256 全等 + `reviewAnnotationsNewFiles` 为空 + 平台 run 活体锚定） |
| cmd-06 | 可达 | **exit 1 / 1 failure**（`evidence/pi-source.json` 未落地，重跑实测 **0.453～0.467 s**）；**Pi 侧基础设施本节点已实跑就位**：clone 21.5 s、`npm ci` 2 m 14 s、hydrate 4.9 s、`build:offline` 9.7 s、探针复跑 2.7 s | 变更后须 failures = 0（变更文件集 + 用例复跑 + 日志 sha256 + 安装包负向） |
...
>
> **基线时点（防把基线数当门禁数）**：表内各行的「本条 run 实测」是**本行字面 args 的最后一次实跑值**，其 KB diff 面与 failures 计数随 `plan.md`／`tasks/**` 的落盘与提交而变化——`cmd-04`／`cmd-05` 两行已在 dev-plan 回修轮（B-2／B-3 修复后、按回修后的字面 args）重跑刷新，`cmd-03` 行为未改 args 的原始基线。M4 期复跑时以**当次实测**为准，不得把本表数字当门禁阈值比对。
...
**AC-6 的 daemon PID 面子项（显式登记，非静默漏项）**：SDD §6.2 的 AC-6 行把「Multica daemon PID 前后一致」列为可观测结果，但单元 B 的验收落在 Pi 工具进程（`createBashTool` 的本地执行路径），**不启动、不接触任何 daemon**：`multica` daemon 与本 CR 的 Pi 侧改动无调用关系，其 PID 的观测既不在 §6.2 六条证据命令的观测面内，也不属本 CR 的变更面。故该子项在本计划内**不适用（N/A，有据）**，判据落在父/后代 PID 不存在与既有 `killProcessTree(child.pid)` 路径（TASK-02 §4 用例 2，`cmd-06` 活体复跑），daemon 生命周期稳定性由「本 CR 不启停／不重启任何共享服务」的边界（§5.0 不适用声明）保证，不作为 AC-6 的独立验收项，也不为此新增观测设施（I5）。
...
| 产出方 | **本团队构建产物**（本地检出 + `npm run build:offline`），版本随检出的 `packages/coding-agent/package.json` 版本号 | `dep-1` §1.3.2 路线表 R-C 行 |
| 升级动作时点 | **CR 合并之后**，属运行环境维护（替换 PATH 上的 `pi` 可执行文件）；不进 CR 交付 diff | `dep-1` §1.3.2 共用边界 2 |
| 交付证据形态 | 检出 URL + 基线 SHA + 检出形态（`detached@v0.85.1`＝ detached HEAD @ `d981de12`，**无分支**，受控 git 入口不提供建分支形态）＋ HEAD commit + 变更文件清单 + 用例执行日志（`evidence/pi-source.json` + `evidence/pi-vitest.log`） | SDD §1.3／§4.6 |
...
| 后续可选 | 检出（detached HEAD）上的提交日后可作为 fork/PR 的推送源（不改本 CR 的任何设计与判据） | SDD §1.3 证据形态的路线无关性 |
