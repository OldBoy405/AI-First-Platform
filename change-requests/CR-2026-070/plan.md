...
| cmd-05 | ≤ 60 s（实测 **0.036 s** 于证据缺席态，B-5 回修轮复跑；含平台查询预期 ≤ 20 s） | §6.3 干跑 |
...
**行内自校（B-5 回修项，实施期与回修期同判据）**：本表每行必须逐字满足 `竖线数 = 7`（⇒ 恰 6 个 cell：`证据ID`／`repo`／`cwd`／`executable`／`args`／`timeout`），且 `args` cell 必须能 `JSON.parse` 成 token 数组、四条 `-e` 命令的源串必须**零**裸双引号／反引号／反斜杠／竖线——任一不成立即**表结构已被破坏**（行被切成多于 6 个 cell），此时该 cell 已不是字面值、不得据被截断的片段重建命令算法，必须先修表再转录。布尔或语义不得写成裸 `||`，改写为等价无竖线形态（本表 `cmd-05` 的 AC-4 空数组断言即取 `!(A && B)` 形态）；确需竖线字面量时以 `String.fromCharCode(124)` 构造。回修轮实测（`plan.md` HEAD `2b9353cf` → 本次提交）：改前 `cmd-05` 行竖线数 9／cell 数 8、`JSON.parse` 报 `Unterminated string in JSON at position 2823`；改后六行全部 7 竖线／6 cell／`JSON.parse` 通过、四条 `-e` 源串零违规。
...
>
> **基线时点（防把基线数当门禁数）**：表内各行的「本条 run 实测」是**本行字面 args 的最后一次实跑值**，其 KB diff 面与 failures 计数随 `plan.md`／`tasks/**` 的落盘与提交而变化——`cmd-04`／`cmd-05` 两行已在 dev-plan 回修轮（B-2／B-3／B-5 修复后、按回修后的字面 args）重跑刷新，`cmd-03` 行为未改 args 的原始基线（B-5 轮复跑仅用于复核失败项逐条一致，不重置基线）。M4 期复跑时以**当次实测**为准，不得把本表数字当门禁阈值比对。
