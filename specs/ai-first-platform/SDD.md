...
该类型实现 `error`、`Unwrap() error` 与 `slog.LogValuer`。`Unwrap()` 保留既有 `errors.As` / `isTransientError()` 结构化分类；`Error()` 保留既有 complete 永久拒绝后构造 fail payload 的功能语义；`LogValue()` 只返回 `task_id`、`terminal_kind`、`error_class` 三个稳定脱敏字段，防止原始 cause 被 logger 展开。
...
- `terminalReportFailure` 的 `Unwrap()` 使既有 `isTransientError()` 继续对原始 cause 分类；`Error()` 仅供已有 complete→fail fallback 生成发往服务端的 fail `errorMessage`，其原始错误文本绝不传给 logger；`slog.LogValue()` 只暴露 `task_id`、`terminal_kind`、`error_class`，因此新补投逻辑不会把原始 cause 展开到日志。
- 仅当发送的最终错误为瞬时错误时，`reportTerminalTask` 才调用 `enqueueTerminalReport(copyImmutable(report))`，并返回包含该原始 cause 的安全包装。永久错误不入队，亦返回安全包装；新增补投逻辑不记录原始 cause、errorMessage、output、session、workdir 或完整 report。现有 caller 的非敏感上下文属性保持既有行为，不在本 CR 内逐个改造 caller。
...
**终态日志合同覆盖初次投递、complete→fail fallback 与重放三条路径。** 新增补投逻辑及 `terminalReportFailure` 只能记录 task ID、终态种类和稳定的错误类别，不记录原始错误文本、`errorMessage`、`output`、session/workdir、完整 report 或会话内容。`terminalReportFailure.LogValue()` 只控制错误属性的表示；本 CR 不新增共享 logger handler，也不逐个改造现有 caller。已有 caller 的非敏感上下文属性保持原有日志行为，但其 error 属性必须使用该安全包装或等价安全值。日志顺序固定为：complete 失败日志 → 仅永久错误发送 fail → fallback 失败日志；该收敛只改变新增错误值的日志表示，不改变既有服务端 payload、错误分类或重试/入队语义。
...
reportTerminalTask(parentCtx, report):
  ...
  return failure  // LogValue 仅暴露 task_id/terminal_kind/error_class；不改变既有 caller 的非敏感上下文属性

handleSuccessfulResult(ctx, completeReport):
  err = reportTerminalTask(ctx, completeReport)
  if err == nil: return
  // Existing caller may pass non-sensitive task/status attrs; LogValue keeps the error field safe.
  log existing caller error with err
  ...
  if fallbackErr != nil: log existing caller error with fallbackErr
...
| 日志边界 | `terminalReportFailure.LogValue()` 只对新增错误属性输出 `task_id`、`terminal_kind`、`error_class`；不记录原始 cause 或终态 payload，现有 caller 不逐个改造 | 共享 logger handler 会扩大 multica 上游修改面；逐个 caller 改造违反 FR-18 且增加 rebase 面 |
...
| FR-16~FR-17 | 复用 `deliverTerminalTaskReport` 和服务端幂等接口；root context 停止；`terminalReportFailure.LogValue()` 对新增错误属性提供稳定脱敏字段，覆盖初次/fallback/重放/冲突的新增日志，不记录原始 cause 或 payload | payload 保真、关闭、错误值及终态 payload 不泄露测试 |
| FR-18 | 新 Go 文件/测试、`daemon.go` 两处最小挂钩、`CUSTOM.md` 登记；不修改 `server/internal/logger`，不逐个改造 complete/fail caller | diff 范围、日志脱敏、台账与 Go 测试审查 |
...
- P2 日志决策覆盖新增终态补投逻辑的初次投递、complete→fail fallback、重放和冲突：`terminalReportFailure.LogValue()` 只生成 `task_id`、`terminal_kind`、`error_class` 三个稳定脱敏字段；不得记录或展开原始 cause、errorMessage、output、session/workdir、完整 report 或会话内容。现有 caller 的非敏感上下文属性保持既有行为，本 CR 不新增共享 logger handler，也不逐个改造 caller。
- `terminalReportFailure.Error()` 仅在 complete 永久拒绝的已有 fallback 中构造**发往 FailTask 的**服务端 `errorMessage`；该文本不得作为 logger 的原始 error 输出。初次 complete、fallback fail、重放和冲突日志均使用安全错误值或安全字段，并保持“complete 失败日志 → 仅永久错误发送 fail → fallback 失败日志”的顺序。
...
- 使用捕获 `slog.Handler` 验证初次投递失败、complete permanent→fallback fail、fallback fail 失败、重放和冲突路径：断言新增错误值不包含 output、errorMessage、session、workdir 和原始 cause；断言 `LogValue()` 的字段只有 `task_id`、`terminal_kind`、`error_class`。现有 caller 的非敏感上下文属性不作为本 CR 的新白名单要求。额外断言 `errors.Unwrap` / `isTransientError` 与 complete→fail fallback 仍获得原始 cause，证明日志脱敏未改变功能语义。
...
| 上游 rebase 冲突 | 新增逻辑放独立 daemon 文件；`daemon.go` 仅增加零值 retry 字段和 `reportTerminalTask` 的错误包装两个小型 AIFIRST 挂钩；不修改 `server/internal/logger`，现有 caller 不逐个改造；`CUSTOM.md` 留痕 | 修改共享 logger、修改每个 complete/fail caller、改变普通日志输出或全局 logger 行为 |
...
本设计不触及 `crctl.mjs` dispatch 分支或 `controlled-shell/rules.json` deny 面，因此不触发 Prompt 采纳影响小节。后续 SDD 评审应重点核对：严格模式是否保持默认兼容、候选校验是否确在 write-set 前、complete 瞬时不 fallback 且永久 fallback 的发送/日志顺序是否明确、`terminalReportFailure.LogValue()` 是否只暴露稳定脱敏错误字段且不泄露原始 cause/payload、初次/fallback/重放终态日志是否保持安全、P2 nullable 行号约定是否落实、以及三域是否都在 §4.3 的既有证据闭环中具备实际产物。
