...
status: pending
estimate: 2h
depends-on: [CR-2026-050-TASK-13]
...
仓根只允许按 `execution_context.resources[].repo` 匹配后取对应 `worktreePath`；以下均为仓根相对路径：

- `repo=tools: skills/shared/crctl/scripts/test/pipeline-structure.test.mjs`（DD-7 提交前缀扫描断言）
- `repo=tools` + `repo=multica`：在各自 worktree 运行验证，无其他代码改动
...
3. multica worktree：`node server/internal/governance/gen/generate-gate-nodes.mjs --tools <tools-worktree> --check`；随后 `cd server && go test ./internal/governance/`。
...
上述 4 条验收全部通过，并执行 `crctl task done CR-2026-050 --task CR-2026-050-TASK-14 --workspace {execution_context.operational_workspace}` 更新受控索引；禁止手写 `tasks/_index.yml`。
