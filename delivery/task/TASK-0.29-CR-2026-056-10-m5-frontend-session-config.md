...
4. **Private Ask 面板（FR-3/FR-12，BLOCK-007）**：去掉只读模型徽章，改可写 Model/Thinking，走 `PATCH /api/chat/sessions/{id}/config`（creator-only）；不写 Team Agent session；历史缺 `base_*` 的 `agent_default` 展示照常（后端语义）。GET `/api/projects/{projectId}/private-chat` 响应经新独立 schema 解析：后端响应 = `ChatSession` 字段 + `session_id`（= `id`，同一 UUID）+ `model`/`thinking_level`/`*_source`；client 以 `session_id` 为 PATCH/发送凭据（即 `id` 的**命名映射**，不改写不截断）。
5. **文案**：新增文案四语齐（`parity.test.ts` 绿，AC-26）。

## 验收条件

1. `packages/core/api/schemas.test.ts`：硬降级 → 全量 fallback 且只读；软默认 → `*_source="session_default"` 且可写；无伪造 `session_id`（AC-27）。**Private Ask GET 映射夹具（BLOCK-007）**：合法 UUID `session_id` 逐字符保留（与响应 `id` 相等）；缺 `session_id` / 空 / 非 UUID → 整体硬降级到 `UNSAFE_CHAT_CONFIG_FALLBACK`（只读、禁 PATCH/发送、重试 GET）；缺 model/source → 软默认可写。
2. `packages/views/locales/parity.test.ts` 绿（AC-26）。
3. 组件测：`persistModel`/Thinking 持久化后 `api.updateAgent` 无调用（spy 断言，AC-1/AC-2）；两成员 Private Ask 互不影响（AC-3）；picker 可写且 draft/附件/发送/停止/重试不回归（AC-21）；非 owner/admin 控件只读态渲染；Private Ask 面板以响应 `session_id` 构造 PATCH 请求（BLOCK-007）。
...
- 消费：TASK-06 GET/PATCH 响应形状（§3.1：`session_id`、可空 `issue_id`、`team_agent_id`、`model`、`thinking_level`、`model_source`、`thinking_level_source`；PATCH 三态请求体）；TASK-13 Private Ask GET/PATCH 响应形状（`ChatSession` + `session_id`（= `id`）+ 四附加字段，BLOCK-007）；TASK-07 messages 请求体（必带 `session_id`）与成功体 `{session_id, issue_id, comment_id, task_id}`、container 200 体、merge-forward 附加字段；TASK-09 上传省略 `issue_id` 的后端容忍；基线 `parseWithFallback`（`packages/core/api/schema.ts`）、`ModelPicker` / `ThinkingPicker`。
