...
3. **发送（§4.6，BLOCK-005 接缝 + BLOCK-006 范围封闭 + BLOCK-009 事务边界）**：`SendChatMessage` → `SendDirectChatMessage`：
   - **分支判定与快照绑定同一锁内行（BLOCK-009，权威算法）**：取消事务外 Resolve/校验预检。固定序列 = 基线既有 `runInTx`（task.go:2402 函数内）→ `LockChatSessionForRuntimeBind`（FOR UPDATE，事务首语句，chat.sql:390）→ `GetChatSession` 锁内重读 → 状态检查（基线 :2445 原位保留）→ **分支判定** → 分支内动作（Private Ask：回填 → Resolve → §4.3 校验 → 快照构造；普通：零动作）→ **基线 carrier 重读 + 守卫（task.go:2448-2456 原位：`GetAgentForClaimUpdate`（agent.sql，FOR UPDATE）→ `RuntimeID.Valid == false` 即 `ErrChatTaskAgentNoRuntime`）** → `HasPendingChatTurnForSession` → `CreateChatTask` → commit。分支判定、Resolve、§4.3 校验、回填与 `chat_config` 快照全部基于**锁后重读行 `currentSession`**（同一行、同一事务、同一份 Resolve 输出），任一步失败整体 rollback。本 TASK 只在该锁后插入分支与动作：不新增锁查询、不改变基线锁序（§4.14 锁序首条仍是该行锁）。**分支动作先于 carrier 重读是验收 10 失败注入 b 夹具可执行的前提**：回填/catalog I/O 期间事务仅持 session 行锁（agent 行锁在 carrier 重读才取得），并发生产解绑（`UPDATE agent`）可提交；carrier 重读保持在入队前最后一刻（MUL-5163「锁内重读 carrier」语义不变）。代价：unbound 的 Private Ask 发送会先完成回填 + 校验再被守卫拒绝——整体回滚、零残留，无脏数据。
...
10. **事务边界与并发夹具（BLOCK-009/010，分支判定与快照绑定锁内行）**：
    - **project clear ∥ send（BLOCK-010，两顺序各自为可执行夹具，逐项断言，不得仅引用 BLOCK-008 的 PATCH 夹具代替）**：历史 Private Ask session（`base_*` NULL、无 override、`project_id` 非空）上并发项目删除的 `ClearChatSessionProjectByProject`（chat.sql:18，普通 UPDATE）与发送（`LockChatSessionForRuntimeBind` FOR UPDATE）——同行走锁，天然串行；顺序由测试控制（clear 以测试事务执行同款 UPDATE 并控制提交点；catalog 时序以 latch fake `ChatCatalogPort` 控制），每顺序按「成功 / 失败注入」两个子夹具断言：
      - **① clear 先提交（普通路径）**：send 锁后重读 `project_id` 已 NULL → 按普通聊天路径处理。**成功子夹具**：发送成功；事务后 DB 断言 `base_model`/`base_thinking_level` 单一一致值 = 仍 NULL（普通路径不回填）、任务行 `context` 为 NULL（无快照，与锁内普通分支判定一致）、消息行与任务行存在且 `task_id` 关联正确（发送结果与锁内行状态一致）。**失败注入 a（catalog 不应触发）**：强制 `CacheLoad` miss + `LiveLoad` 失败——发送仍成功、无 `invalid_model_or_thinking_level` 400（普通路径完全不触 catalog I/O）。**失败注入 b（入队点失败 = 事务内 carrier 守卫，task.go:2448-2456）**：发送前用**生产解绑时序**使 carrier 不可入队——`UnbindUserAgentsFromRuntime`（runtime.sql:355：`agent.runtime_id = NULL`，migration 251 语义「NULL = unbound」）→ `UnbindTasksFromRuntime` → `DeleteAgentRuntime`（`agent_runtime_id_fkey` 为 `ON DELETE RESTRICT`（migration 004:72-73），必须先解绑置 NULL 删除才可达；生产删除流程即此序，handler/runtime.go:820→855→881）。随后发送：锁 → 重读 → 分支 = 普通路径 → 基线 carrier 重读 `GetAgentForClaimUpdate`（task.go:2448，FOR UPDATE）读到 `RuntimeID.Valid == false` → task.go:2455-2456 返回基线错误 `ErrChatTaskAgentNoRuntime`（非本 CR 新增错误码）→ 整体 rollback。事务后 DB 断言零残留：`base_*` 仍 NULL、无任务行（无孤儿任务）、无消息行、`updated_at` 未变（`TouchChatSession` 已回滚）；随后**重绑**（新 runtime 行 + 生产 agent 更新路径写回 `runtime_id`）后重发立即成功（行锁随回滚释放）。**边界声明**：该注入触发的是事务内 carrier 重读守卫，不是 `CreateChatTask` 的 INSERT——不得再称「删 runtime = INSERT 失败机制」（发送事务自 carrier 重读起持 agent 行 `FOR UPDATE`，任何让 fence 谓词为假的 owner 行消失都必须先提交对该 agent 行的更新，被发送事务自己的锁阻塞；`agent_runtime_id_fkey` RESTRICT 使 runtime 行在引用存在时不可删；fence false 分支生产上只由 teardown/merge 竞态触发且互锁表现为死锁中止，非确定）。INSERT 0 行语义改在查询边界夹具验证（②b 后半）。
      - **② send 先提交（project-bound 路径）**：锁内行 `project_id` 非空 → Private Ask 路径。**成功子夹具**：发送成功；`base_model`/`base_thinking_level` 单一一致值 = 回填恰一次的当时 Agent 默认（两列各一值，无第二处写入）；任务 `context.chat_config` 快照 = 锁内 Resolve 输出（无 override → 等于回填值，与校验共用同一份解析输出、禁止二次解析）；**快照与事后 GET/DB 一致**：事务后断言会话行 `base_*` == 任务快照 == clear 前 Private Ask GET 响应的 `model`/`thinking_level`（`*_source=session_default`）；随后 clear 提交把 `project_id` 置 NULL——再次 DB 断言已落库快照/回填/override 值均不可变。**失败注入 a（catalog，发生于回填之后）**：`CacheLoad` miss + `LiveLoad` 返回错误/超时 → `400 invalid_model_or_thinking_level` → 整体 rollback；事务后 DB 断言零残留：`base_*` 仍 NULL（无部分回填）、无任务行（无孤儿任务）、无消息行。**失败注入 b（入队点失败，回填后触发）**：同一生产解绑时序，以 latch 控制使其发生在**回填之后**（固定序列：分支动作先于 carrier 重读，见实现要点 3）——发送事务在锁内回填后停在 `LiveLoad`（cache miss，latch fake `ChatCatalogPort`），此时事务仅持 session 行锁（agent 行锁要到 carrier 重读才取得）；测试连接提交解绑（`UPDATE agent` 不与 session 行锁冲突）+ 删除 runtime（RESTRICT 已由解绑满足）；释放 latch 并返回**合法**模型列表（证明失败不是 catalog 所致）→ §4.3 校验通过、快照构造 → carrier 重读读到 NULL → task.go:2455-2456 `ErrChatTaskAgentNoRuntime` → **整体 rollback 覆盖已发生的回填**；零残留断言同上（`base_*` 仍 NULL = 回填已回滚、无任务行、无消息行、`updated_at` 未变）；随后重绑 + 重发立即可进（行锁随回滚释放）。**INSERT 0 行语义在查询边界夹具**（复用基线 `TestLockTaskOwnerRows_RejectsUnresolvableOwner` 模式，workspace_delete_fence_test.go:738）：解绑 + 删 runtime 后直接调 `qtx.CreateChatTask`（同参数形状，fence 谓词因 runtime owner 消失为假）→ 0 行 → `pgx.ErrNoRows`——证明该接缝「INSERT 0 行 + sqlc `:one` 错误传导」，与发送级回滚夹具合起来覆盖完整契约。
