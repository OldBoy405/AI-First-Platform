...
> 决策状态：**conditional（条件触发）**。本文件是候选设计输入，不是当前实施授权；只有具备明确 owner、维护频率和稳定检索需求后，才按 `docs/product/部分/缺口清单-最终版.md` 的准入规则注册 CR。正文中的阶段、工期和交付切分均为历史设计假设，注册时必须依据当前代码与权威合同重新核实。
> 权威边界：当前能力处置以 `docs/product/部分/缺口清单-最终版.md` 为准（该文件 `:202` 将 Wiki maintain/query 列为条件触发候选，`:221` 明确排除「Wiki 与组织智能的一体化大包」）；状态机、Pipeline、Skill 和 `crctl` 行为继续以 tools 权威文件为准。
> 承接：v1.1 知识层设计（PRD §2.1.7 三分工：事实生产 / 知识消费 / 知识晋升）、P0 §6.1 目录契约扩展、P2.5 runner 轻量档。**知识晋升（§5）自 2026-08-19 起由本文单独拥有**（原在 P3 §2.3）。
...
> **修订：2026-08-19 —— 接收 P3 §2.3 知识晋升巡检的完整所有权**（P3 同日删除其 §2.3 与交付项 E9，理由见《P3 组织智能设计》§7 R-12：两个信号源、执行体、产出全在 Wiki 侧，拆两处产生跨文档循环依赖）。同时：修正两处失效路径（缺口清单实在 `docs/product/部分/` 下）、§4.3 的 `spec_trace` 改为 `cr_sync_event`（该表已随 P3 R-10 砍除）、§5 补入原 P3 §2.3 的执行模型与幂等键细节。
...
| **晋升**（高频缺口回写事实源） | 定时巡检（knowledge-agent 只读） | **本文内，W4；不在 P3 计划内** |
...
- **`wiki_query_log`**（P2.5 落库；**表形制由本文拥有**）：只记问题文本 + 命中路径 + 提问人 + 时间，**不记答案正文**——答案是衍生物，问题才是信号（存答案只会多一份会过期的副本，而晋升判定从不读它）。建表时遵 multica 仓硬规则：禁新增 FK、索引必 `CREATE INDEX CONCURRENTLY` 且一个迁移文件一条。

### 4.3 治理类查询分流

同一入口（spec-query Agent / `/进度` 斜杠命令）后面分流：CR 状态、追溯链、审批记录等**治理事实** → 直查 `cr` / `cr_sync_event`（追溯走 `event_kind='trace'` 的 payload；**原文写的 `spec_trace` 投影表已随 P3 R-10 砍除**）/ `approval_record`（权威、无转述）；架构解释、模块行为、"为什么这么设计" → wiki-first 三层。分流规则写在 Agent prompt 里，按问题意图判别，含混时先答治理事实再补 Wiki 解释。

---

## 5. 知识晋升（本文单独拥有，双信号源）

> 本节自 2026-08-19 起为知识晋升的**单一事实源**（原 P3 §2.3 已删，其交付项 E9 从 P3 移除）。

“当一个问题被反复问到，就应该推动它回到事实源，而不是永远停留在问答记录里。”晋升信号来自两路：

| 信号源 | 抓什么 | 来源 |
|---|---|---|
| **信号一：问答日志聚类** | **被反复问的**——近 30 天同类问题 ≥3 次且命中文件分散（文件分散说明事实源没有一处权威回答；「同类」由巡检时 knowledge-agent 对 30 天问题列表做语义分组**现场判定**——零新基建，不建向量库不算相似度） | `wiki_query_log` |
| **信号二：open-questions.md 的 Active 条目** | **生成时就答不上的**——wiki-maintain 维护 Wiki 时发现的缺口（缺口在生产侧就被记录，不用等有人来问） | wiki-maintain 运行产物（§2.4） |

两路汇进同一晋升流：周巡检（knowledge-agent **只读**）→ 两路信号汇总去重 → 自动开**普通 Issue**「建议将〈主题〉回写事实源」，附问题样本 / open-question 条目 + 建议落点（specs/ 或 docs/）→ 回写走人审的正常 CR/文档流程。**只开 Issue 不动文件**——巡检判定的是“这个主题应该有权威答案”，而“权威答案是什么”不得由巡检自己写。被回写解决的 open-question 移入 Answered 节并链接落点。

**幂等键 = 主题归一化文本**：同一主题 30 天内已开且未关闭的 Issue 不重复开（Issue 内记录样本问题 id）。没有这个键，一个长期无人回写的高频主题会每周开一个新 Issue，一个月后建议本身成为噪声源。

这补全了知识层的回路：**事实源（git）→ 消费（Wiki 问答）→ 晋升（高频问答与生产缺口回写事实源）**。

### 5.1 执行模型与已知脆弱点（开工前必读）

执行链路：服务端 `sys_cron_executions` 派发 → 共享队列 → daemon 侧 knowledge-agent 认领执行（只读）→ 开 Issue。某周期无 daemon 在线则**顺延到下一周期，不补跑不积压**。

**脆弱点必须先说清**：这条链路与《P3 组织智能设计》§7 R-11 推迟的两个 LLM 巡检（`review-alignment` / `change-impact-analysis`）**是同一种结构**：多跳 + 依赖“某台 daemon 恰好在线” + LLM 不确定性，任一跳断了都是静默无结果，而**“无结果”与“无晋升候选”在外部看起来完全一样**。因此本项与那两个巡检同等待遇：均不在当前实施授权内，要做就得先有 daemon 在线率的真实数据，且必须同时交付一个“上次巡检实际执行于何时”的可见指示（同 P1 §B.4 对 EVIDENCE_DRIFT 的要求：必须能区分“从未发生过”与“从未测过”）。
...
| 问答日志 | `wiki_query_log`（P2.5 落库） | **表形制由本文拥有**（§4.2）；本设计不另建表 |
| 写白名单 | `rules.json#taskWriteAllowlist` | P1 单一事实源文件加一段 |
| 晋升信号二 | `wiki/open-questions.md` | 文件即信号，巡检读取 |
| 晋升产出 | 普通 Issue | 走现有 Issue 域，无新表无新状态机 |

除 `wiki_query_log`（随 P2.5 问答能力一同建）外无新 PG 表、无新权威域、无新事件类型——符合"治理层做成新增文件"纪律。（原文此处声称的“零新表”严格讲不成立：`wiki_query_log` 本身就是一张新表，只是它不属于晋升而属于问答。）
...
| W4 | Wiki 问答 prompt（wiki-first 三层 + 出处 + 分流 + 不可信证据） | P2.5 | ① 问答回答带路径+行号出处 ② 治理类问题直查 `cr`/`cr_sync_event` 不经 Wiki |
| **W5** | 知识晋升巡检（§5 双信号源，**原 P3 E9**） | 后置（见 §5.1 脆弱点；与 P3 R-11 推迟的两个 LLM 巡检同等待遇） | ① 人为对同一主题重复提问 3 次 → 周巡检自动开出「建议回写事实源」Issue，附问题样本与建议落点 ② `open-questions.md` 的 Active 条目出现在同一巡检的晋升 Issue 里 ③ **同一主题 30 天内不重复开 Issue**（幂等键生效） ④ **看得到“上次巡检实际执行于何时”**（区分无候选与未执行） |

排期归属：W1 是文档工作（随 P0 文档修订）；W2/W3 主体在 P2.5 之后（约 +2 周，daemon 侧与 runner 侧可并行）；W4 并入既有 P2.5 问答条目；**W5 单列且后置**——原文将它“并入 P3 §2.3 巡检条目”的写法自 2026-08-19 失效（P3 已无此条目）。
